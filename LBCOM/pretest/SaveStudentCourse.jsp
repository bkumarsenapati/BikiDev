...
<%
	...
	try
	{
		...
		rs1=st1.executeQuery("select * from pretest_lms where course_id='"+courseId+"' and school_id='"+schoolId+"'");
		while(rs1.next())
		{
	...
	//devCourseId="DC001";				///////// ---------->  Important
		...
		rs=st1.executeQuery("select * from dev_course_master where course_id='"+devCourseId+"' and status=1");
		if(rs.next())
		{
		...
		{
			rs=st.executeQuery("select image_name from dev_copy_images where image_path='general'");
		...
		{
			rs1=st1.executeQuery("select image_name from dev_copy_images where image_path='course'");
			while(rs1.next())
			{
...
%>
    ...
    <tr>
      <td width="64%" colspan="2" align="center" rowspan="2" height="34">
		...
		<a href="javascript:window.close();"><font color="black">CLOSE</font></a>
      </td>
    </tr>
</table>
</center>
