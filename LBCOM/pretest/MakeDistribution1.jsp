...
<%
	...
	ResultSet rs=null,rs1=null,rs8=null;
	...
	{	 
		...
		lesIds = request.getParameterValues("lids");
		if (lesIds != null)
		{
			for (int i = 0; i < lesIds.length; i++)
			{
				if(i==0)
				{
					lessonStr=lesIds[i];
		...
		System.out.println("For unitStr...."+unitStr);
		assgnIds = request.getParameterValues("asgnids");
		if (assgnIds != null)
		{
			for (int i = 0; i < assgnIds.length; i++)
			{
				if(i==0)
				{
					widStr=assgnIds[i];
				...
				{
					widStr=widStr+","+assgnIds[i];
				}
			}
			
		}
		System.out.println("For widStr...."+widStr);
		assmtIds = request.getParameterValues("assessids");
		if (assmtIds != null)
		{
			for (int i = 0; i < assmtIds.length; i++)
			{
				if(i==0)
				{
					assessStr=assmtIds[i];
				}
				else
				{
					assessStr=assessStr+","+assmtIds[i];
				}
				//
			}
			
		}
		System.out.println("For assessStr...."+assessStr);
...
//  status int(1)  (0-Assigned; 1- Exam Paper Seen; 2- Submitted; 3-     ;  4- Evaluated;)
		...
		System.out.println("sId is in MakeDistri11111..."+sId);
		int i=0;
		st8=con.createStatement();
		rs8=st8.executeQuery("select * from pretest_student_material_distribution where course_id='"+courseId+"' and student_id='"+sId+"' and school_id='"+schoolId+"'");
		if(rs8.next())
		{
			// This entry is already there
			i=st7.executeUpdate("update pretest_student_material_distribution set lesson_ids='"+lessonStr+"',unit_ids='"+unitStr+"',assignment_ids='"+widStr+"',assessment_ids='"+assessStr+"' where course_id='"+courseId+"' and student_id='"+sId+"' and school_id='"+schoolId+"'");

		}
		else
		{
			i=st7.executeUpdate("insert into pretest_student_material_distribution(school_id,student_id,course_id,unit_ids,lesson_ids,assignment_ids,assessment_ids,status) values ('"+schoolId+"','"+sId+"','"+courseId+"','"+unitStr+"','"+lessonStr+"','"+widStr+"','"+assessStr+"','0')");
		...
		response.sendRedirect("DistributeAssessments.jsp?asgncount="+widLen+"&assignedcount="+assignedCount+"&ignoredcount="+ignoredCount+"&studentids="+sidStr+"&studentid="+sId+"&startdate="+startDate+"&duedate="+dueDate);
