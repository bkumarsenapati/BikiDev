...
<%
	...
	{	 
		...
		System.out.println("widStr..."+widStr);
		...
		*/
		...
		System.out.println("select * from pretest_student_material_distribution where course_id='"+courseId+"' and student_id='"+studentId+"' and school_id='"+schoolId+"");

		st2=con.createStatement();
		rs2=st2.executeQuery("select * from pretest_student_material_distribution where course_id='"+courseId+"' and student_id='"+studentId+"' and school_id='"+schoolId+"'");
		if(rs2.next())
		{
			lidStr=rs2.getString("lesson_ids");
			widStr=rs2.getString("assessment_ids");
			unitStr=rs2.getString("unit_ids");
			asgnStr=rs2.getString("assignment_ids");
		}
		System.out.println("lidStr..."+lidStr);
		System.out.println("widStr..."+widStr);
		System.out.println("unitStr..."+unitStr);
		System.out.println("asgnStr..."+asgnStr);
...
//  status int(1)  (0-Assigned; 1- Exam Paper Seen; 2- Submitted; 3-     ;  4- Evaluated;)
		...
		for(Enumeration e2 = studentIds.elements() ; e2.hasMoreElements() ;)
		{
			...
			if(flag==true)
			{
				...
				st1.executeUpdate(qryStr);
				ignoredCount1++;
				System.out.println("ignoredCount1....."+ignoredCount1);
	...
	// ----------------> Make Available start here
		...
		System.out.println("SaveStudentCourse.jsp?asgncount="+assignLength+"&assignedcount="+assignedCount2+"&ignoredcount="+ignoredCount2+"&assmtcount="+assessLength+"&assmtassignedcount="+assignedCount1+"&assmtignoredcount="+ignoredCount1+"&studentids="+sidStr+"&startdate="+startDate+"&duedate="+dueDate);

		response.sendRedirect("SaveStudentCourse.jsp?asgncount="+assignLength+"&assignedcount="+assignedCount2+"&ignoredcount="+ignoredCount2+"&assmtcount="+assessLength+"&assmtassignedcount="+assignedCount1+"&assmtignoredcount="+ignoredCount1+"&studentids="+studentId+"&startdate="+startDate+"&duedate="+dueDate);
