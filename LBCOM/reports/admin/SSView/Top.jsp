...
function goclass(){
	var classid=document.getElementById("classid");	window.location="Top.jsp?m_id=<%=marking_id%>&classid="+classid.value+"&classdes="+classid.options[classid.selectedIndex].text+"";
...
function gocourse(){
	...
	if((courseid.value!="")&&(classid.value!=""))	
		parent.BodyFrame.location="Body.jsp?m_id=<%=marking_id%>&classid="+classid.value+"&classdes="+classid.options[classid.selectedIndex].text+"&courseid="+courseid.value+"&coursedes="+courseid.options[courseid.selectedIndex].text;
	else
		parent.BodyFrame.location="about:blank";
	}
//-->
...
<table border="1" cellspacing="0" style="border-collapse: collapse" id="table1" width="100%">
  <tr>
    <td>
	<%
	rs=st.executeQuery("select class_id,class_des from class_master where school_id='"+schoolId+"'  and class_id= any(select distinct(class_id) from coursewareinfo where school_id='"+schoolId+"' and status=1)");
	%>&nbsp;&nbsp; <b>Class&nbsp; </b>&nbsp;
	...
	<%
	rs=st.executeQuery("select course_name,course_id from coursewareinfo where school_id='"+schoolId+"'and class_id='"+classId+"' and status=1");
	%>
