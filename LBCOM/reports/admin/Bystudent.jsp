...
function init(){
	if("<%=classId%>"==="null")
		var x=document.getElementById("classid").value="ALL"
	else
...
function goteacher(select){
	window.location="Byclass.jsp?m_id=<%=marking_id%>&teacherid="+select.value+"";
...
<tr>
	...
	%>
    <td width="59%">
	<select size="1" name="classid" id="classid" onchange=gocourse(this)>
		<%
   ...
   <td width="213">
    <p align="right"><b><font size="1" face="Verdana"><a href="javascript:window.print()">Print&nbsp;&nbsp;
</a> </font><font size="2" face="Verdana">&nbsp;</font></b></td>
   </tr>
  <%
...
%>
	...
	<BR>
	<table border="1" cellspacing="0" id="AutoNumber3" width="158" bgcolor="#eeba4d" cellpadding="0" style="border-collapse: collapse">
	  <tr>
	...
	<table border="1" cellspacing="0" width="100%" id="AutoNumber1" bordercolor="#808000" cellpadding="0" bordercolorlight="#808000" style="border-collapse: collapse">
	  <tr>
		<td width="156" height="17"  width=150 bgcolor="#EEE0A1">
		<p align="center"><b><font face="Verdana" size="2">Student&nbsp;ID</font></b></td>
		<%
		rs=st.executeQuery("select course_name,course_id from coursewareinfo where school_id='"+schoolId+"' and class_id='"+classId+"'");
		%>
		<%while (rs.next()){
			cources.add(rs.getString("course_id"));%>
			<td width="816" height="17" align="center" bgcolor="#EEE0A1">
			<p align="center"> <b> <font face="Verdana"size="2"><%=rs.getString("course_name")%></font></b></td>
		<%}%>	
	  </tr>
		<%	rs=st.executeQuery("select distinct det.student_id as student_id ,cwi.class_id from coursewareinfo_det det,coursewareinfo cwi where cwi.school_id=det.school_id and cwi.course_id=det.course_id and cwi.school_id='"+schoolId+"' AND cwi.class_id='"+classId+"' AND det.student_id <> '"+classId+"_vstudent ' Order by student_id");	
		%>
		<%while (rs.next()){%>	
		  <tr>
			...
			<%
				...
				rs1=st1.executeQuery("select course_name,course_id from coursewareinfo where school_id='"+schoolId+"' and class_id='"+classId+"'");
				while (rs1.next()){
