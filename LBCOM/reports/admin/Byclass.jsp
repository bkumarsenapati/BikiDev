...
function init(){
	if("<%=teacherId%>"==="null")
		var x=document.getElementById("teacherid").value="ALL"
	else
		var x=document.getElementById("teacherid").value="<%=teacherId%>"
...
function gocourse(select){
	...
	window.location="Byclass.jsp?m_id=<%=marking_id%>&teacherid="+teach.value+"&classid="+select.value+"&classdes="+select.options[select.selectedIndex].text+"";
...
<tr>
	...
	%>
	<td >
		<select size="1" id="teacherid" name="teacherid" onchange=goteacher(this)>
			<%
			if(!rs.next()){%>
				<option value="ALL" >No teachers are added yet</option>
			<%}else{%>		
