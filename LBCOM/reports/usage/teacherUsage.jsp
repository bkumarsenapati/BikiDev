...
<%
 ...
 %>	
	<table border='1' width='100%' cellspacing='0' bordercolordark='#DDEEFF' height='20'>
...
<%
	...
	rs.close();
	rs=st.executeQuery("select grade, username, fname, lname from studentprofile where schoolid='"+schoolId+"'  and crossregister_flag in(0,1,2) and grade= any(select distinct(class_id) from coursewareinfo where teacher_id='"+teacherId+"' and school_id='"+schoolId+"')");
	int i=0,j=1;
	while (rs.next()) {
		...
		ExceptionsFile.postException("teacherUsage.jsp","operations on database","Exception",e.getMessage());
    }finally{
		try{
			if(st!=null)
				st.close();
			...
			ExceptionsFile.postException("teacherUsage.jsp","closing statement and connection  objects","SQLException",se.getMessage());
