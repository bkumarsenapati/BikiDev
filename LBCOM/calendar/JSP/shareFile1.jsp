...
<form name="grstudselectfrm" id='gr_stud_id' action="#" onsubmit="return share('test');"><BR>
...
<%
	...
	{
		e.printStackTrace();
		ExceptionsFile.postException("shareFile1.jsp","operations on database","Exception",e.getMessage());
    }
	finally
	{
		try
		{
			...
			se.printStackTrace();
			ExceptionsFile.postException("ShareFile1.jsp","closing statement and connection  objects","SQLException",se.getMessage());
...
function back()
{
	document.location.href="EditEvent.jsp?uid=<%=uid%>";
}
</script>
</html>