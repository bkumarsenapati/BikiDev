...
public class  AssLBCMSVariations extends HttpServlet
{
	public void init(ServletConfig conf) {
		...
		{
			ExceptionsFile.postException("AssLBCMSVariations.java","init","Exception",e.getMessage());
		}
	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws IOException,ServletException{
        ...
        try{
			...
			path="/LBCOM/lbcms/AssmtBuilder/"+courseId+"/"+examId+"/";
			
			out.println("window.location.href=\"/LBCOM/lbcms/AssmtBuilder/QuestionsList.jsp?view=yes&courseid="+courseId+"&exId="+examId+"\";");
			out.println("</script>");	

		}catch(Exception e){
			ExceptionsFile.postException("AssLBCMSVariations.java","doPost","Exception",e.getMessage());
		}finally{
			try{
				 if(st!=null)
					 st.close();
				 ...
				 sortedList=null;
				        ...
				        ExceptionsFile.postException("AssLBCMSVariations.java","closing connections","SQLException",se.getMessage());
               }
		}
	}
	...
	private String sortGroupWise(String quesString,int size){
		try	{
			...
			ExceptionsFile.postException("AssLBCMSVariations.java","sortGroupWise","Exception",e.getMessage());
		}
		return quesString;
	}
	private String randomize(Hashtable quesList){
		...
		try{
			...
			ExceptionsFile.postException("AssLBCMSVariations.java","randomize","Exception",e.getMessage());
		}
		return qString;
	}
	private int getSize(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("AssLBCMSVariations.java","getSize","Exception",e.getMessage());
		}finally{
			stkQues=null;
		}
		return size;
	...
	private Hashtable getQuestionsList(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("AssLBCMSVariations.java","getQuestionsList()","Exception",e.getMessage());
		}finally{
			stkQues=null;
		}
		return quesList;
	...
	private Hashtable formQuestion(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("AssLBCMSVariations.java","formQuestion","Exception",e.getMessage());
		}
		return grpsList;
	}
	...
	private Hashtable sortQuesWise(Statement st,ResultSet rs,Hashtable questionsList,String schoolId,String classId,String courseId){
		...
		try{
			...
			ExceptionsFile.postException("AssLBCMSVariations.java","sortQuesWise","Exception",e.getMessage());
		}
		return sortedList;
		//return ques;
	  }
	  ...
	  private Vector getSortedList(Hashtable qList){
		  ...
		  try{
			...
			Collections.sort(list);
				...
				ExceptionsFile.postException("AssLBCMSVariations.java","getSortedList","Exception",e.getMessage());
		 }
	  ...
	  private String getSortedQuesString(Hashtable qList,Vector list){
		  ...
		  try{
			...
			//	sortedList.put(i,qList.get(i));
				...
				ExceptionsFile.postException("AssLBCMSVariations.java","getSortedQuesString","Exception",e.getMessage());
		  }
		return q;
	  }
		  ...
		  try{
			 ...
			 // Hashtable qList=new Hashtable();
			  ...
			  while(ele.hasMoreElements()){
				...
				ExceptionsFile.postException("AssLBCMSVariations.java","getSortedHashtable","Exception",e.getMessage());
		  }
		  return sortedList;
	  }
