...
public class  LBCMSAssBuilderVariations extends HttpServlet
{
	public void init(ServletConfig conf) {
		...
		{
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","init","Exception",e.getMessage());
		}
	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws IOException,ServletException{
		...
		Statement st=null;
		ResultSet rs=null;
		LBCMSAssBuilderExamPaper examPaper=null;
        ...
        try{
			...
			String schoolPath=null,path="",tableName="";
			int random=0,sort=0,variations=0,size=0,noOfGrps=0;
			
			ServletContext application = getServletContext();
		    schoolPath = application.getInitParameter("lbcms_dev_path");
			...
			db=new DbBean();
			examPaper=new LBCMSAssBuilderExamPaper();	
			...
			rs=st.executeQuery("select * from "+tableName+" where assmt_id='"+examId+"'");
			while (rs.next())
			{
			...
			//out.println("parent.parent.top_fr.location.href=\"/LBCOM/exam/CETopPanel.jsp?status=4&editMode=edit&examId="+examId+"&examName="+examName+"&examType="+examType+"&noOfGrps="+noOfGrps+"\";");

			//path=schoolPath+"/"+courseId+"/"+examId+"/";
			path="/LBCOM/coursedeveloper/AssmtBuilder/"+courseId+"/"+examId+"/";
			
			out.println("window.location.href=\"/LBCOM/coursedeveloper/AssmtBuilder/QuestionsList.jsp?view=yes&courseid="+courseId+"&exId="+examId+"\";");
			out.println("</script>");	

		}catch(Exception e){
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","doPost","Exception",e.getMessage());
		}finally{
			try{
				 if(st!=null)
					 st.close();
				 ...
				 sortedList=null;
				        ...
				        ExceptionsFile.postException("LBCMSAssBuilderVariations.java","closing connections","SQLException",se.getMessage());
               }
		}
	}
	...
	private String sortGroupWise(String quesString,int size){
		try	{
			...
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","sortGroupWise","Exception",e.getMessage());
		}
		return quesString;
	}
	private String randomize(Hashtable quesList){
		...
		try{
			...
			while(grpEle.hasMoreElements()){
				...
				while(stk.hasMoreTokens()){
					...
					//Collections.shuffle(questions);		
																	// Questions are  jumbled removed by Santhosh
				}
			...
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","randomize","Exception",e.getMessage());
		}
		return qString;
	}
	private int getSize(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","getSize","Exception",e.getMessage());
		}finally{
			stkQues=null;
		}
		return size;
	...
	private Hashtable getQuestionsList(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","getQuestionsList()","Exception",e.getMessage());
		}finally{
			stkQues=null;
		}
		return quesList;
	...
	private Hashtable formQuestion(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","formQuestion","Exception",e.getMessage());
		}
		return grpsList;
	}
	...
	private Hashtable sortQuesWise(Statement st,ResultSet rs,Hashtable questionsList,String schoolId,String classId,String courseId){
		...
		try{
			...
			if(queryString.length()>0){
				...
				rs=st.executeQuery("select q_id,q_type from lbcms_dev_assmt_content_quesbody where ("+queryString+")");
				while(rs.next()){
			...
			ExceptionsFile.postException("LBCMSAssBuilderVariations.java","sortQuesWise","Exception",e.getMessage());
		}
		return sortedList;
		//return ques;
	  }
	  ...
	  private Vector getSortedList(Hashtable qList){
		  ...
		  try{
			...
			Collections.sort(list);
				...
				ExceptionsFile.postException("LBCMSAssBuilderVariations.java","getSortedList","Exception",e.getMessage());
		 }
	  ...
	  private String getSortedQuesString(Hashtable qList,Vector list){
		  ...
		  try{
			...
			//	sortedList.put(i,qList.get(i));
				...
				ExceptionsFile.postException("LBCMSAssBuilderVariations.java","getSortedQuesString","Exception",e.getMessage());
		  }
		return q;
	  }
		  ...
		  try{
			 ...
			 // Hashtable qList=new Hashtable();
			  ...
			  while(ele.hasMoreElements()){
				...
				ExceptionsFile.postException("LBCMSAssBuilderVariations.java","getSortedHashtable","Exception",e.getMessage());
		  }
		  return sortedList;
	  }
