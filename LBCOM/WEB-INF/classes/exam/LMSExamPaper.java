...
public class LMSExamPaper 
{
	...
	public void setPaper(Hashtable verns,String examId,int nQtns,String teacherId,String qtnTbl,String schoolId,String courseId,String courseName,String schoolPath,int importFlag)
	{
		try
		{
			...
			//System.out.println("Entered into exampaper..."+examId);
			this.qtnTbl=qtnTbl;
			...
			rs=st.executeQuery("select * from "+examTblName+" where exam_id='"+examId+"' and school_id='"+schoolId+"'");
			if(rs.next()) 
			{
			...
			if(noOfGrps>0)
			{
				...
				rs1=st1.executeQuery("select group_id,instr,any_all,tot_qtns,ans_qtns,weightage,neg_marks from "+grpTblName+" order by group_id");
				while (rs1.next()) 
				{
					if(!rs1.getString("group_id").equals("-"))
					{
						grpInstr=" Answer ";
						
						int totQtns=rs1.getInt("tot_qtns");						
						int ansQtns=rs1.getInt("ans_qtns");
						int weightage=rs1.getInt("weightage");
						int negMarks=rs1.getInt("neg_marks");

						String marks;
						
						if (rs1.getString("any_all").equals("0"))
						{
						...
						{
							grpInstr=grpInstr+" any "+rs1.getInt("ans_qtns")+" out of "+ rs1.getInt("tot_qtns") +" questions. &nbsp; &nbsp; <font face='Arial' size='2' color='#800000'>("+ansQtns+" X "+weightage+" = "+ansQtns*weightage+")";						
							totMarks=totMarks+(ansQtns*weightage);
						}
						grpIds.put(rs1.getString("group_id"),grpInstr);
						grpInfo.put(rs1.getString("group_id"),new Integer(ansQtns));
			...
			//System.out.println("after construct paper");
		}
		catch(Exception e)
		{
	...
	private void constructPaper(Hashtable versns)
	{
		...
		if (importFlag==1)
		{
			dPath= schoolPath+"/"+schoolId+"/"+teacherId+"/"+"coursemgmt"+"/"+courseId+"/"+"exams/"+examId;
...
//		exTPFile.close();
	...
	private void crtExmMidPanel(String qIds,String vNo,String exmPath)
	{
		...
		try
		{
//System.out.println("qIds all list"+qIds);
			...
			oText=oText+"<script type=\"text/javascript\" src='http://ajax.googleapis.com/ajax/libs/jquery/1.4/jquery.js'></script> <script type=\"text/javascript\">$(document).ready(function(){$('#ar').click(function(){var len = document.q_paper.elements.length;for(var i = 0 ; i< len;i++){if(document.q_paper.elements[i].type =='textarea'){document.getElementById(document.q_paper.elements[i].id).value=tinyMCE.get(document.q_paper.elements[i].id).getContent();}}});});setInterval(function () {parent.mid_f.document.q_paper.ar.click();}, 10000);</script>\n";
			...
			//System.out.println("qListTkn bfr"+qListTkn);
			while(qListTkn.hasMoreTokens())
			{					
				//System.out.println("qListTkn aft"+qListTkn);
				StringTokenizer qIdsTkn=new StringTokenizer(qListTkn.nextToken(),":");	
				//System.out.println("qIdsTkn aft"+qIdsTkn);
				qId=qIdsTkn.nextToken();
				//System.out.println("qId aft"+qId);
...
//System.out.println("qIdsTkn aft"+qIdsTkn);
			...
			oText=oText+"<input type='hidden' id='ar' name='ar' value='submit'>\n";
