...
public class AssLBCMSCreateQuestion extends HttpServlet{	
	...
	public void service(HttpServletRequest request,HttpServletResponse response){
		...
		try{
			...
			String qtnBdyTbl="lbcms_dev_assmt_content_quesbody";
			...
			//	}
				...
				if (flag==1)
				{
					...
					ps=con.prepareStatement("insert into "+qtnBdyTbl+"(course_id,assmt_id,q_id,q_type,q_body,ans_str,hint,c_feedback,ic_feedback,difficult_level,possible_points,incorrect_response,estimated_time,time_scale,status) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		   ...
		   else
			{
				...
				ps=con.prepareStatement("update "+qtnBdyTbl+" set q_body=?,ans_str=?,hint=?,c_feedback=?,ic_feedback=?,difficult_level=?,estimated_time=?,time_scale=?,possible_points=?,incorrect_response=? where q_id=?");
				...
				ps.setInt(9,PointsPossible);
				ps.setInt(10,IncorrectResponse);
				ps.setString(11,qId);
				ps.execute();
		   }
