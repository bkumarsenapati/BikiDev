...
public class ExamPaper 
{
	...
	public void setPaper(Hashtable verns,String examId,int nQtns,String teacherId,String qtnTbl,String schoolId,String courseId,String courseName,String schoolPath,int importFlag)
	{
		try
		{
			...
			rs=st.executeQuery("select exam_name,exam_type,dur_hrs,dur_min,instructions,no_of_groups from "+examTblName+" where exam_id='"+examId+"' and school_id='"+schoolId+"'");
			if(rs.next()) 
			{
...
//Santhosh
		...
		{
			ExceptionsFile.postException("ExamPaper.java","setPaper","Exception",e.getMessage());
		}
		finally
		{
			try
			...
			{
				ExceptionsFile.postException("ExamPaper.java","closing connections","SQLException",se.getMessage());
            }
		}
	}
	...
	private void constructPaper(Hashtable versns)
	{
		...
		if (importFlag==1)			
			...
			dPath= schoolPath+"/"+schoolId+"/"+teacherId+"/coursemgmt/"+courseId+"/exams/"+examId+"_tmp";
...
//		fStr=fStr+"	var qTypes=parent.mid_f.qType;\n";
	 ...
	 public void beginAnsHtml(String path){
		...
		try{
			...
			str=str+"		var arr=option.split('~');"+"\n";  //   Ramanujam changed split(',') to split('~') on 18-11-06
			...
			str=str+"	top.mid_f.document.getElementsByName(id)[0].value=option;"+"\n";
			str=str+"}"+"\n";
