...
//before destroying the object, database connection will be closed
  ...
  public Connection getConnection()throws ClassNotFoundException,IOException,SQLException{  
	...
	try{
			...
			DataSource ds = (DataSource)ctx.lookup("java:comp/env/jdbc/hsnWHDB");
			if (ds != null) 
  ...
  public void close(Connection dbCon){
	  try{
			...
			ExceptionsFile.postException("SQLBean.WHDbBean.java","close","SQLException",e.getMessage());
			
		}catch(Exception e){
			ExceptionsFile.postException("SQLBean.WHDbBean.java","close","Exception",e.getMessage());
