...
<%
	...
	Statement st=null;
	ResultSet rs=null;
	String emailid="",foldername="",schoolid="",tag="",newfoldername="",idx="",status="";
	int flag=0,i=0;
%>
<%
	...
	try
	{
		...
		rs=st.executeQuery("select foldername from teacherpersonaldocs where emailid='"+emailid+"' and schoolid='"+schoolid+"'");
...
%>
		<!--<table border="0" cellspacing="1" width="100%" id="AutoNumber1">
		  <tr>
		    <td width="4%"><input type="radio" value="<%= foldername %>" name="fold" onclick="javascript:parent.displaymain.location.href='../asm/ShowFiles.jsp?foldername=<%=foldername%>&emailid=<%=emailid%>&schoolid=<%=schoolid%>'"><b><font size="1.5" face="Verdana"><img border="0" src="images/Folder.gif" width="15" height="13"></font></b></td>
		    <td width="96%"><b><font size="1.5" face="Verdana">
            <a href="/LBCOM/asm/ShowFiles.jsp?foldername=<%=foldername%>&emailid=<%=emailid%>&schoolid=<%=schoolid%>" target="displaymain" onclick="javascript:checkRadio(<%= i %>);"><%=foldername%></a>&nbsp;</font></b></td>
		  </tr>
  		  <% //i++;  %>
		</table>-->
		...
		<table border="0" cellspacing="1" width="100%" id="AutoNumber1">
		  <tr>
			<td width="10%"><input type="radio" value="<%= foldername %>" name="fold" onclick="javascript:parent.displaymain.location.href='/LBCOM/asm/ShowFiles.jsp?foldername=<%=foldername%>&emailid=<%=emailid%>&schoolid=<%=schoolid%>'">
			</td>
			<td width="10%" align="left">
					<img border="0" src="images/Folder.gif" width="15" height="13">
			</td>
			<td align="left"><font face='arail' size='2'>
					<a href="/LBCOM/asm/ShowFiles.jsp?foldername=<%=foldername%>&emailid=<%=emailid%>&schoolid=<%=schoolid%>" target="displaymain" onclick="javascript:checkRadio(<%= i %>);"><%=foldername%></a>&nbsp;</font>
			</td>
		  </tr>
		  <% i++;  %>
		</table>
				...
				if(tag!=null){
					...
					if(tag.equals("ren")){
						...
						if (status.equals("success")){
							out.println("parent.displaymain.location.href='/LBCOM/asm/ShowFiles.jsp?foldername="+newfoldername+"&emailid="+emailid+"&schoolid="+schoolid+"';");
						...
						if(status.equals("exist"))
							out.println("alert('Folder already exists with this name');");	
							out.println("parent.displaymain.location.href='/LBCOM/asm/ShowFiles.jsp?foldername="+foldername+"&emailid="+emailid+"&schoolid="+schoolid+"';");
							out.println("document.forms[0].elements["+ --i +"].checked=true;");
					}
					else if(tag.equals("del")){

						if(foldername.equals(""))
							out.println("parent.displaymain.location.href='/LBCOM/asm/selectpersonalfolder.html';");
						else{
						out.println("parent.displaymain.location.href='/LBCOM/asm/ShowFiles.jsp?foldername="+foldername+"&emailid="+emailid+"&schoolid="+schoolid+"';");
						out.println("document.forms[0].elements["+ --i +"].checked=true;");
						}
...
<%
	...
	{
		ExceptionsFile.postException("LeftDir.jsp","Operations on database and reading parameters ","Exception",e.getMessage());
		out.println(e);
	}
		...
		if (con!=null && !con.isClosed())
			...
			ExceptionsFile.postException("LeftDir.jsp","closing connection and statement objects","Exception",ex.getMessage());
