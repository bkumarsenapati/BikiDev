...
%>
<%
    String userId="",schoolId="",queryString="",folderName="",attachPath="",loginType="";
		    ...
		    while(sd1.hasMoreTokens())
		    {
			  ...
			  for(int z=0;z<hiddenList.size();z++)
			  {
				    ...
				    String e_mail= (String)hiddenList.get(z);
	        	   	    if((size1/1048576) < 250)      // user space size limit is 250MB
			   	    {
...
%>
 ...
 <td width="50%" height="20" align="right">
			<font face="verdana" size=1 color="#000000"><b>&nbsp;&nbsp;&nbsp; <%=space%> used  of total 250MB</b></font>
		</td>
		...
		<%
			  ...
			  {
 %>
            <td width="350" height="20"><font face="arial" size=1 color="#AA5303">&nbsp;&nbsp;&nbsp; <%=space%> used  of total 100MB</font></td>
<%           
	          } 
            }
		   else{  
...
<% 
	System.out.println("Entered into Inbox.jsp at 1");
...
<%
	...
	System.out.println("Entered into Inbox.jsp at 2");
%>
			</font>
		</td>
	</tr>
...
<%
			System.out.println("Entered into Inbox.jsp at 3");
		rs = st.executeQuery("select distinct folder from mail_folder where user_id='"+emailId+"' order by folder");
        while(rs.next())
		{
			System.out.println("Entered into Inbox.jsp at 4 in whle rs.next");
%>					  
...
<%
	int i=0;		 
	System.out.println("Entered into Inbox.jsp at 5");
...
<%
    ...
    {
		System.out.println("Entered into Inbox.jsp at 6");
%>	    
		<a href="Inbox.jsp?start=0&folder=<%=folderName%>&sorttype=D&sortby=id" target="_self">
<%   
		if(folderName.equals("Sent"))
		{
			System.out.println("Entered into Inbox.jsp at 7");
%>
...
<%   			       
		if(folderName.equals("Sent"))
		{
			System.out.println("Entered into Inbox.jsp at 8");
%>
...
<%
    ...
    {
		System.out.println("Entered into Inbox.jsp at 9");
%>	    
...
<%
	...
	{
		System.out.println("Entered into Inbox.jsp at 10");
%>	    
...
<%
	System.out.println("Entered into Inbox.jsp at 11");
	rs = st.executeQuery("select from_user,to_user, subject, receive_date,email_id, attachment, mark_read from mail_box where   user_id='"+emailId+"' and folder='"+folderName+"' and mark_read in(0,1) order by "+sortStr+" LIMIT "+start+","+pageSize);
		...
		while(rs.next())
		{
			System.out.println("Entered into Inbox.jsp at 12 in while rs.next");
			...
			System.out.println("Entered into Inbox.jsp at 13");
			...
			System.out.println("Entered into Inbox.jsp at 14");

			System.out.println("fromString in Inbox.jsp at 15 is.."+fromString);
			System.out.println("fromString length in Inbox.jsp at 15 is.."+fromString.length());
			...
			System.out.println("Entered into Inbox.jsp at 16");
			
			checkboxValue = rs.getString("email_id");
%>	
			<tr bgcolor="#FFD89D">
			<td align="right"><input type="checkbox" name="selids" value='<%=checkboxValue%>'></td>
...
<%  
			...
			{
%>
				<img border="0" src="images/attach_21.jpg" width="15" height="19">
<%  
			}
			if(bflag==true)
			{
...
<%  
	...
	{
		System.out.println("Exception in Inbox.jsp is...."+e);
