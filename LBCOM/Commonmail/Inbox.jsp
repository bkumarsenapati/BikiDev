...
<%
	...
	int totRecords=0,start=0,end=0,c=0,pageSize=50,currentPage=0;
		    ...
		    while(sd1.hasMoreTokens())
		    {
			  ...
			  for(int z=0;z<hiddenList.size();z++)
			  {
				    ...
				    String e_mail= (String)hiddenList.get(z);
	        	   	    if((size1/1048576) < 500)      // user space size limit is 500MB
			   	    {
...
%>
 ...
 <td width="50%" height="20" align="right">
			<font face="verdana" size=1 color="#000000"><b>&nbsp;&nbsp;&nbsp; <%=space%> used  of total 500MB</b></font>
		</td>
		...
		<%
			  ...
			  {
 %>
            <td width="350" height="20"><font face="arial" size=1 color="#AA5303">&nbsp;&nbsp;&nbsp; <%=space%> used  of total 250MB</font></td>
<%           
	          } 
            }
		   else{  
	...
	rs = st.executeQuery("select from_user,to_user, subject, receive_date,email_id, attachment, mark_read from mail_box where   user_id='"+emailId+"' and folder='"+folderName+"' and mark_read in(0,1) order by "+sortStr+" LIMIT "+start+","+pageSize);
		...
		while(rs.next())
		{
			...
			if(folderName.equals("Sent"))
			{
				...
				frmUser=rs.getString("from_user");
...
<%  
			...
			{
%> 
				<a href="#" onclick="viewmail('<%=start%>','<%=folderName%>','<%=sortingBy%>','<%=sortingType%>','<%=checkboxValue%>','<%=frmUser%>')">  <b><%=subjectString%></b></a>
<% 
			}
			else
			{
%>
				<a href="#" onclick="viewmail('<%=start%>','<%=folderName%>','<%=sortingBy%>','<%=sortingType%>','<%=checkboxValue%>','<%=frmUser%>')">  <%=subjectString%></a>
<%  
			}
%>
			</font>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function viewmail(start, foldername, sortby, sorttype, detail,fromuser){
	               window.location.href="ViewMail.jsp?start="+start+"&sortby="+sortby+"&sorttype="+sorttype+"&folder="+foldername+"&detail="+detail+"&fromuser="+fromuser;	
	      return false;
	}
