/*HM_ScriptDOM.js
* by Peter Belesis. v4.0.9 010501
...
HM_NS7 = (navigator.vendor == ("Netscape7") || navigator.product == ("Mozilla"));
...
function HM_f_AssignTreeParametersInteger(arrayvalue,defaultvalue){
	var ValueIsString = (typeof arrayvalue == "string");
	if (ValueIsString) arrayvalue = arrayvalue.trim();
	var ValueIsNull = ((arrayvalue == null) || (typeof arrayvalue == "undefined") || (ValueIsString && arrayvalue.length == 0));
	return (ValueIsNull) ? defaultvalue : arrayvalue;
}
...
function HM_f_MakeTrees(){
	...
	for(var t=0; t<HM_a_TreesToBuild.length; t++) {
		...
		HM_CurrentTree = {
			...
			MenuLeft         : MenuLeft = HM_f_AssignTreeParametersInteger(TreeParams[1],null),
			MenuTop          : MenuTop = HM_f_AssignTreeParametersInteger(TreeParams[2],null),
...
function HM_f_MakeItemElement(menucount) {
	...
	if(ItemElement.linkText) {
		...
		if(HM_ShowLinkCursor)ItemElement.style.cursor = HM_NS6 ? "pointer" : "hand" && HM_NS7 ? "pointer" : "hand";
	}
		...
		with(ImageElement){
			...
			with(style) {
				if(HM_CurrentTree.RightToLeft) {
					...
					if(HM_NS6 || HM_NS7) cssFloat = "right";
...
function HM_f_SetItemStyle() {
	with(this.style){
		...
		font = ((HM_FontBold) ? "bold " : "normal ") + HM_FontSize + "px " + HM_FontFamily;
...
function HM_f_PopMenu(e){
	...
	HM_CurrentMenu.xPos = (HM_CurrentMenu.tree.MenuLeft!=null) ? eval(HM_CurrentMenu.tree.MenuLeft) : EventX;
	HM_CurrentMenu.yPos = (HM_CurrentMenu.tree.MenuTop!=null)  ? eval(HM_CurrentMenu.tree.MenuTop)  : EventY;
	HM_CurrentMenu.xIntended = HM_CurrentMenu.xPos = (HM_CurrentMenu.tree.MenuLeft!=null) ? eval(HM_CurrentMenu.tree.MenuLeft) : EventX;
	HM_CurrentMenu.yIntended = HM_CurrentMenu.yPos = (HM_CurrentMenu.tree.MenuTop!=null)  ? eval(HM_CurrentMenu.tree.MenuTop)  : EventY;
...
function HM_f_ItemOver(){
    ...
    if (this.hasMore) { 
		...
		if (this.tree.PositionUnder && (this.menu == this.tree.treeParent)) {
			this.child.xPos = parseInt(this.menu.style.getTrueXPosition) + parseInt(this.style.left);
			this.child.yPos = parseInt(this.menu.style.getTrueXPosition)  + this.menu.offsetHeight - (HM_BorderWidth);
		}
		else {
			if(HM_IE5M) {
	...
	for(var i=0; i<HM_TotalTrees; i++) {
		...
		if(TopMenu.style.visibility == "visible") {
		    ...
		    while(TempMenu.hasChildVisible) {
				...
				if(!TopMenu.tree.TopIsPermanent) TempMenu.keepInWindow();
