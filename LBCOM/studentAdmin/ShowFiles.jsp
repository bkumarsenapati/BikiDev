...
<%
	...
	String foldername=null,emailid=null,schoolid=null,folderpath=null,filepath=null;
%>
...
function deletedoc()
{
 ...
 <table align=center border="0" cellspacing="0" width="75%" id="AutoNumber3"  style="border-collapse: collapse" bordercolor="#111111" cellpadding="0">
	...
	<td align="right">
	<b><font size="2" face="Verdana"><a href="javascript:deletedoc()">[Delete a document]</a></font></b>
	</td>
 	</tr>
	</table>
	</td>
 	...
 	<tr>
	 	<td bgcolor="#E08040" colspan=3 align="left" height=20><font size="2" face="Verdana"><b><%=foldername%></b></font></td>
 	</tr>
	<tr> 
    <td colspan="3">&nbsp;</td>
  </tr>
...
<%
		if(flist.length!=0)
		{
			for(int i=0;i<flist.length;i++)
			{
				...
				//String httppath= "../schools/"+schoolid+"/"+emailid+"/PersonalFolders/"+foldername+"/"+ flist[i];
				String httppath=(String)session.getAttribute("schoolpath")+schoolid+"/"+emailid+"/PersonalFolders/"+foldername+"/"+ flist[i];
%>
  	<tr>
	 	<td width="10%" align="left"><input type="radio" value="<%= files.getName()%>" name="file%>" style="float: right"></td>
	 	<td width="70%" align="left">
		
		
		
		<a href="javascript:na_open_window('win', '<%=httppath%>', 0, 0, 600, 400, 1, 0, 1, 1, 1)" target="_self">
		<!--<a href="<%= httppath %>" target="new">--><b><font face="Verdana" size="1.5"><%= flist[i] %></font></b></a></td>
