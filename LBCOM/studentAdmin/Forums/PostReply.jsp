...
<%
	...
	String ruser="",posteddate="",sno="",topicid="";
...
<FORM action="/LBCOM/forums.StudentReplyForum?sno=<%=sno%>&topicid=<%= topicid%>&sid=<%=schoolid%>&fid=<%=forumid%>&topic=<%=topic%>&user=<%=auser%>&puser=<%=puser%>&postdate=<%= posteddate %>&dir=<%= dir %>&crtby=<%=crtby%>&utype=<%=utype%>&desc=<%=desc%>&topiccount=<%=topiccount%>&replycount=<%=replycount%>&ruser=<%=ruser%>&replydate=<%=replydate%>&type=2" METHOD="POST" enctype="multipart/form-data" NAME="PostReply" onSubmit = "return Validate();" >
