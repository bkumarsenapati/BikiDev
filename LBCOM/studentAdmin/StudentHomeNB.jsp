...
<%	
	try
	{
		...
		while(rs.next())
		{
			...
			{
				query1="select distinct * from notice_master where dirname='"+nbName+"' and schoolid='"+schoolId+"' and user_type!=1 and (imp='1'||imp='2') and from_date<=curdate() and to_date>=curdate() order by from_date";
			}
			else
			{
				query1="select distinct * from notice_master where dirname='"+nbName+"' and schoolid='"+schoolId+"' and courseid IN (select course_id from coursewareinfo_det where school_id='"+schoolId+"' and student_id='"+studentId+"') and user_type!=1 and (imp='1'||imp='2') and from_date<=curdate() and to_date>=curdate() order by from_date";
			}
...
<%			st2=con.createStatement();
				...
				else
				{
					...
					{
						response.sendRedirect("NBTop1.jsp?nbid="+nbId);
					}
				}
				st2.close();
			}
