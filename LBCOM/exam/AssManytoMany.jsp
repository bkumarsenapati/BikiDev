...
<%
	...
	{	 
		...
		workIds=request.getParameter("examid");
		...
		{ 
			...
			//rs=st.executeQuery("select user_id from "+schoolId+"_cescores as ces inner join coursewareinfo_det cd on cd.student_id=ces.user_id and cd.school_id='"+schoolId+"' and cd.course_id='"+courseId+"'  where ces.school_id='"+schoolId+"' and work_id='"+examId+"' and report_status!=2");
	...
	{
		System.out.println("The exception in AssManyToMany.jsp is....."+se.getMessage());
	}
	catch(Exception e)
	{
...
{	
	...
	else
	{
		...
		else
		{
			...
			{
				window.location.href="AssSelectDates.jsp?workids=<%=workIds%>&studentids="+studentids;
				return true;
			}
		}
	}
...
function goNextPrev(s,tot)
{
	...
	document.location.href="AssManyToMany.jsp?workids=<%=workIds%>&start="+start+"&totrecords="+totalrecords;
}
...
function gotopage(totrecords)
{
	...
	else
	{
		...
		parent.main.location.href="AssManyToMany.jsp?workids=<%=workIds%>&start="+start+"&totrecords="+totrecords;
		return false;
	}
}
...
{
	window.open("ShowAssessmentsList.jsp?studentid="+sid,"Document","resizable=no,scrollbars=yes,width=550,height=400,toolbars=no");
}
</SCRIPT>
...
<tr>
	...
	<td width="35%" bgcolor="#C0C0C0" height="21" align="center"><b>
		<font face="Verdana" size="2" color="#003399">No. of Assessments</font></b>
	</td>
</tr>
...
<%
		try
		{
			...
			{		
				...
				rs1=st1.executeQuery("select count(*) from "+tableName+" as s inner join exam_tbl as e inner join "+schoolId+"_cescores as c on s.exam_id=e.exam_id and e.school_id='"+schoolId+"' and e.course_id='"+courseId+"' and e.status='1' and c.school_id='"+schoolId+"' and c.work_id=e.exam_id and user_id='"+studentId+"' and c.report_status=1");
				if (rs1.next())
