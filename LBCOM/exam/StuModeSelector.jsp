...
<%
	...
	try
	{
		...
		//teacherId="teacher1";
		System.out.println("...................................In StuModeSel..."+teacherId);
		schoolId=(String)session.getAttribute("schoolid");
		courseId=(String)session.getAttribute("courseid");
				
		tableName=request.getParameter("examtbl");
...
<%
	...
	{
		ExceptionsFile.postException("StuModeSelector.jsp","General","Exception",e.getMessage());
		System.out.println("The Error in StuModeSelector.jsp is : "+e);
	}
	finally
	{
		try
		...
		{
			ExceptionsFile.postException("StuModeSelector.jsp","closing statement and connection  objects","SQLException",se.getMessage());
			System.out.println("The Error in StuModeSelector.jsp is : "+se.getMessage());
		}
	}
%>
...
function setSubmissionMode()
{
	...
	{
		parent.modeframe.location.href="StuModeSelector.jsp?mode=SUBMISSION&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&count=<%=count%>&securedmarks=<%=securedMarks%>&teacherid=<%=teacherId%>";
	...
	{
		parent.modeframe.location.href="StuModeSelector.jsp?mode=VIEW&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&count=<%=count%>&securedmarks=<%=securedMarks%>&examtbl=<%=tableName%>&teacherid=<%=teacherId%>";

		parent.btm_f.location.href="../schools/<%=schoolId%>/<%=teacherId%>/coursemgmt/<%=courseId%>/exams/<%=examId%>/bottom.html";
		parent.fb_f.location.href="StuSubmissionDetails.jsp?studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&attempt=<%=count%>&status=<%=status%>&securedmarks=<%=securedMarks%>&mode=submission";
	}
...
function setEditMode()
{
	...
	if(stat==2)
	{
		parent.modeframe.location.href="StuModeSelector.jsp?mode=EVALUATION&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&count=<%=count%>&securedmarks=<%=securedMarks%>&examtbl=<%=tableName%>&teacherid=<%=teacherId%>";
		...
		parent.fb_f.location.href="StuSubmissionDetails.jsp?studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&attempt=<%=count%>&status=<%=status%>&securedmarks=<%=securedMarks%>&mode=reeval";
	}
...
function setViewMode()
{
	...
	parent.fb_f.location.href="StuSubmissionDetails.jsp?studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&attempt=<%=count%>&status=<%=status%>&securedmarks=<%=securedMarks%>&mode=view";
}
//-->
</SCRIPT>
</head>
...
<table border="1" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    ...
    <td width="33%" align="center" bgcolor="#546878">&nbsp;&nbsp;&nbsp;<font size="2" face="verdana" color="#ffffff"><%=mode%> MODE
...
<!-- <table border="1" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td width="100%" bgcolor="#dcdcde" align="left"><b>
