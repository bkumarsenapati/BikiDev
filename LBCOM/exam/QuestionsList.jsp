...
<%
	...
	ResultSet	rs=null;
	String sessid=null;
	String mode="",dbString="",linkStr="",schoolId="";
...
try{
	...
	if (request.getParameter("totrecords").equals("")) {
		   ...
		   if(!(subTopicId.equals("none") && qType.equals("none")))
				dbString="select count(*) from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.sub_topic_id='"+subTopicId+"' and s.q_id=q.q_id and s.q_type='"+qType+"' and q.q_type='"+qType+"' and q.status!='2' order by s.q_id";

		   if((subTopicId.equals("none") && qType.equals("none")))
				dbString="select count(*) from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.q_id=q.q_id  and q.status!='2' order by s.q_id";

		   if((!subTopicId.equals("none") && qType.equals("none")))
				dbString="select count(*) from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.sub_topic_id='"+subTopicId+"' and q.status!='2'  and s.q_id=q.q_id order by s.q_id";

		   if((subTopicId.equals("none") && !qType.equals("none")))
				dbString="select count(*) from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.q_id=q.q_id and s.q_type='"+qType+"' and q.q_type='"+qType+"' and q.status!='2' order by s.q_id";
	   ...
	   if(!(subTopicId.equals("none") && qType.equals("none")))
			dbString="select s.q_id,s.q_type,q.q_body,q.difficult_level from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.sub_topic_id='"+subTopicId+"' and s.q_id=q.q_id and s.q_type='"+qType+"' and q.q_type='"+qType+"' and q.status!='2' order by s.q_id LIMIT "+start+","+pageSize;

	   if((subTopicId.equals("none") && qType.equals("none")))
			dbString="select s.q_id,s.q_type,q.q_body,q.difficult_level from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.q_id=q.q_id and q.status!='2' order by s.q_id LIMIT "+start+","+pageSize;

	   if((!subTopicId.equals("none") && qType.equals("none")))
			dbString="select s.q_id,s.q_type,q.q_body,q.difficult_level from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.sub_topic_id='"+subTopicId+"' and s.q_id=q.q_id and q.status!='2' order by s.q_id LIMIT "+start+","+pageSize;

	   if((subTopicId.equals("none") && !qType.equals("none")))
			dbString="select s.q_id,s.q_type,q.q_body,q.difficult_level from "+tblName+"_sub s, "+tblName+"_quesbody q where s.topic_id='"+topicId+"' and s.q_id=q.q_id and s.q_type='"+qType+"' and q.q_type='"+qType+"' and q.status!='2' order by s.q_id  LIMIT "+start+","+pageSize;
...
<%	try{
	...
	while(rs.next()){
		...
		String qnTblName=schoolId+"_"+classId+"_"+courseId+"_quesbody";
		out.println("<tr>");
