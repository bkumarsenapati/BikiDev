...
<%
	...
	String statusMsg="";
	String feedBack="";
	String mode="view";	//default mode is view
	String examType="";
	String sName="";
	...
	{
		System.out.println("SQLException in SubmissionDetails.jsp is "+se);
	}
	catch(Exception e)
	{
		System.out.println("Exception in SubmissionDetails.jsp is "+e);
	}
	finally
	{
		try
		...
		{
			System.out.println("SQLException in SubmissionDetails.jsp while closing the connection is "+se);
		}
	}
%>
...
function reEvaluate()
{
	...
	parent.modeframe.location.href="ModeSelector.jsp?mode=EVALUATION&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&count=<%=attemptNo%>";
	...
	load();		//parent.fb_f.location="EvaluateSubmission.jsp?mode=reeval&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&count=<%=attemptNo%>&opoints="+oldpoints;
}
//-->
</SCRIPT>
...
<%
		...
		{
%>
			<td width="14%" height="16"  align="left">
				<b><font face="Verdana" size="2" color="#0000FF">&nbsp;Feedback :</font></b>
				<font face="Verdana" size="1pt" color="#0000FF"><a href="FeedBack.jsp?mode=T&actmode=edit&examid=<%=examId%>&examname=<%=examName%>&studentid=<%=studentId%>&attempt=<%=attemptNo%>&marks=<%=marksSecured%>&markssecured=<%=marksSecured%>&status=<%=status%>" title="Click here to edit the feedback">(EDIT)</a></font>
			</td>
...
%>
			<td width="14%" height="16"  align="left">
				<b><font face="Verdana" size="2" color="#0000FF">&nbsp;Feedback :</font></b>
				<font face="Verdana" size="1pt" color="#0000FF"><a href="FeedBack.jsp?mode=T&actmode=add&examid=<%=examId%>&examname=<%=examName%>&studentid=<%=studentId%>&attempt=<%=attemptNo%>&marks=<%=marksSecured%>&markssecured=<%=marksSecured%>&status=<%=status%>" title="Click here to add the feedback">(ADD)</a></font>
			</td>
			<td width="86%" colspan="3" height="16">
				<font face="Verdana" size="2" color="#FF0000">Click ADD button to add the feedback.</font>
			</td>
<%
		}	
%>
