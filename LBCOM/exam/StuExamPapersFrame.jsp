...
<%
	String examId=null,examName="",studentId=null,stuTblName="",examTable="";;
	int attempts=0;
	int version=1;
%>
<%
	...
	String shortAnsMarks="",teacherId="";
	//Santhosh upto here
	String type;
	...
	if(type.equals("student"))
	{
		...
		examTable=request.getParameter("examTable");
...
%>
	<frameset rows="20,*" border="0">
			<frame name="modeframe" src="StuModeSelector.jsp?mode=EVALUATION&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&count=<%=count%>&examtbl=<%=examTable%>&shortansmarks=<%=shortAnsMarks%>&teacherid=<%=teacherId%>" scrolling="no" marginwidth="0" marginheight="0">
		<frameset cols="100,*" border="0">
			<frame name="papernos" src="StuExamPapers.jsp?examid=<%=examId%>&studentid=<%=studentId%>&examname=<%=examName%>&examtype=<%=examType%>&status=<%=status%>&examtbl=<%=examTable%>&shortansmarks=<%=shortAnsMarks%>&teacherid=<%=teacherId%>" scrolling="auto" marginwidth="0" marginheight="0">
