...
<%
	...
	try
	{
		...
		System.out.println("submissionNo..."+submissionNo);
...
function showFile(attempt,marks,mode,status)
{
	...
	parent.fb_f.location.href="StudentSubmissionDetails.jsp?mode=EVALUATION&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&attempt="+attempt+"&status="+status+"&securedmarks="+marks;

	//The following code is used to set the attempt number and the marks obtained in that attempt in the ModeSelector.jsp
	parent.modeframe.location.href="StuModeSelector.jsp?mode=EVALUATION&studentid=<%=studentId%>&examid=<%=examId%>&examname=<%=examName%>&examtype=<%=examType%>&status="+status+"&count="+attempt+"&securedmarks="+marks+"&examtbl=<%=examTbl%>&teacherid=<%=teacherId%>";
...
function changestatus(count,fileFlag)
{
	...
	window.location="disable_enable_exam.jsp?examid=<%=examId%>&examname=<%=examName%>&maxattempts=0&crdate=-&studentid=<%=studentId%>&courseid=<%=courseId%>&version=<%=version%>&submission_no="+count+"&status="+fileFlag+"&exmInsTbl=<%=examTbl%>&mode=del";	
	return false;
}
//-->
</SCRIPT>
...
<tr>
	<td valign="top">
		...
		<tr>
			<td bgcolor="#B0A890" colspan="3" align="center" width="100%">
				<font face="Arial" size="2"><b><%=studentId%></b></font></td>
		</tr>
		<tr>
			<!-- <td width="15" bgcolor="#C2CCE0" height="20"></td> -->
			<td width="100%" bgcolor="#B0A890" colspan="3" height="20" align="center"><b>
...
%>
		...
		<td colspan="2" bgColor="<%=bgColor%>" align='center' width="100%">
