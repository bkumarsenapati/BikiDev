...
<%
	...
	{	 
		...
		System.out.println("workIds...."+workIds);
		...
		{ 
			...
			System.out.println("select distinct(student_id) from training_C000_c0001_dropbox where start_date IS NOT NULL and end_date IS NOT NULL order by student_id");

			rs=st.executeQuery("select distinct(student_id) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where start_date IS NOT NULL and end_date IS NOT NULL order by student_id");
			while(rs.next())
			{
				//c=Integer.parseInt(rs.getString("student_id"));
				//totRecords=c;
				c++;
			}
			totRecords=c;
...
{	
	...
	else
	{
		...
		{
			window.location.href="UnaassignAssignments.jsp?workids=<%=workIds%>&studentids="+studentids;
...
{
	var start=s;  
    var totalrecords=tot;		document.location.href="UnassignStudentsList.jsp?workIds=<%=workIds%>&start="+start+"&totrecords="+totalrecords+"&sidstr=<%=sidStr%>";
}
...
function gotopage(totrecords,docname)
{
	...
	else
	{
		...
		document.location.href="UnassignStudentsList.jsp?workIds=<%=workIds%>&docname=<%=docName%>&start="+start+"&totrecords="+totrecords;
		return false;
	}
}
...
<tr>
	<td width="100%" height="24" align="left">
		<input type="image" src="images/extendvalidity.png" onclick="validate(); return false;"><font face="verdana" size="2" color="#003399"><B></B></font>
	</td>
</tr>
</table>
...
<%
		try
		{
			...
			rs=st.executeQuery("select distinct(student_id) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox as d where start_date IS NOT NULL and end_date IS NOT NULL order by student_id");
			while(rs.next())
			{		
				out.println("<tr>");
...
<tr>
	<td colspan=8 align="left"><input type="image" src="images/extendvalidity.png" onclick="validate(); return false;"><font face="verdana" size="2" color="#003399"><B></B></font></td>
</tr>
</table>
</form>
</body>
