...
{	
	...
	{
		window.location.href="SetExtensionDateBulk.jsp?workids=<%=workIds%>&studentids="+studentids;
		return true;
	}
}
...
{
	var start=s;  
    var totalrecords=tot;		document.location.href="ExtendingBulkStudentsList.jsp?workIds=<%=workIds%>&start="+start+"&totrecords="+totalrecords+"&docname=<%=docName%>";
}
...
function gotopage(totrecords,docname)
{
	...
	else
	{
		...
		document.location.href="ExtendingBulkStudentsList.jsp?workIds=<%=workIds%>&docname=<%=docName%>&start="+start+"&totrecords="+totrecords;
		return false;
	}
}
