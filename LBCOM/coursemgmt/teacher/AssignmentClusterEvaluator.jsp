...
<%
	...
	boolean flag=false;
	int currentPage=0;
	int eval=0,pending=0,submit=0,rest=0;
%>
<%
...
{	
	parent.main.location.href="AssignmentClusterEvaluator.jsp?start="+start+"&totrecords="+totrecords+"&clid="+clid+"&status=&cat="+cat+"&sortby="+sortby+"&sorttype="+sorttype;
	return false;
}
...
function gotopage(totrecords,cat,clid,sortby,sorttype)
{
	...
	else
	{
		...
		parent.main.location.href="AssignmentClusterEvaluator.jsp?start="+ start+ "&totrecords="+totrecords+"&status=&cat="+cat+"&clid="+clid+"&sortby="+sortby+"&sorttype="+sorttype;	
		return false;
	}
}
...
function goGroups() // Selecting Groups
{
	...
	parent.main.location.href="AssignmentClusterEvaluator.jsp?totrecords=&start=0&cat=all&clid="+clusterId+"&status=";
}
...
function extendValidity(wid)
{
	...
	{
		parent.main.location.href="ExtendingStudentsList.jsp?workid="+wid+"&totrecords=";
		return true;
	}
}
//-->
...
%>
	  ...
	  <table border="1" cellpadding="0" cellspacing="0" bordercolor="white" width="100%">
		<tr>
			...
			<td width="35%" bgcolor="#C0C0C0" align="left">
			...
			<td align="center" width="13%" bgcolor="#C0C0C0">
				<b><font face="Verdana" size="2" color="#003399">Students (<%=tot%>)</font></b>
			</td>
			<td align="center" width="13%" bgcolor="#C0C0C0">
				<b><font face="Verdana" size="2" color="#003399">Submissions</font></b>
			</td>
			<td align="center" width="13%" bgcolor="#C0C0C0">
				<b><font face="Verdana" size="2" color="#003399">Remaining</font></b>
			</td>
		</tr>
		...
		if(rs3.next())
		{
			...
			{					
				...
				while(rs.next())
				{
					...
					rs1=st1.executeQuery("Select count(distinct student_id) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where work_id='"+work_id+"' and status >= 2");
						if (rs1.next())
							submit=rs1.getInt(1);
					
					rs1=st1.executeQuery("Select count(distinct student_id) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where work_id='"+work_id+"' and status >= 4");
						if (rs1.next())
							eval=rs1.getInt(1);
				
					rs1=st1.executeQuery("Select count(*) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where work_id='"+work_id+"' and status >= 2 and status < 4");
						if (rs1.next())
							pending=rs1.getInt(1);
		
					rs1=st1.executeQuery("Select count(*) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where work_id='"+work_id+"' and status >= 2");
						if(rs1.next())
							tot=rs1.getInt(1);
...
%>
					...
					<td width="13%" bgcolor="#EEEEEE" align="center">
						<font size="2" face="verdana" color="<%=forecolor%>">
							<a href="#" onclick="showStudents('<%=work_id%>');return false;"><%=assign%></a>
						</font>
					</td>
					<td width="13%" height="18" bgcolor="#EEEEEE" align="center" valign="middle">
...
<%	
	...
	{
%>	
	<a          		href="StudentsSubmissions.jsp?submsntype=pending&cat=<%=cat%>&workid=<%=work_id%>&totrecords=<%=pending%>&start=0&docname=<%=rs.getString("doc_name")%>&maxmarks=<%=rs.getString("marks_total")%>&alltotrecords=<%=submit%>" title="<%=pending%> submission(s) made by <%=submit%> student(s) pending for evaluation"><%=pending%> (<%=submit%>)</a>
<%	
	}
%>	
		</font>
	...
	<td width="13%" bgcolor="#EEEEEE" align="center">
		<font size="2" face="verdana" color="#003366">
		<a          		href="StudentsNotTaken.jsp?cat=<%=cat%>&workid=<%=work_id%>&totrecords=<%=rest%>&start=0&docname=<%=rs.getString("doc_name")%>&maxmarks=<%=rs.getString("marks_total")%>"><%=rest%></a>
		</font>
	</td>
...
{
	System.out.println("AssignmentClusterEvaluator.jsp operations on database Exception"+e.getMessage());
}
finally
{
		try{
			...
			ExceptionsFile.postException("AssignmentClusterEvaluator.jsp","closing statement and connection  objects","SQLException",se.getMessage());
		}
