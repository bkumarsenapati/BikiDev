...
{	
	...
	else
	{
		...
		else
		{
			...
			{
				window.location.href="SetExtensionDate.jsp?workids=<%=workId%>&studentids="+studentids;
				return true;
			}
			else
			{
				window.location.href="SelectDates.jsp?workids=<%=workId%>&studentids="+studentids;
				return true;
			}
		}
	}
...
<%
		try
		{
			...
			{		
				...
				rs1=st1.executeQuery("select count(distinct work_id) from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where student_id='"+studentId+"'");
				if (rs1.next())
					asgnCount=rs1.getInt(1);
				
				stsClr="#F25B4A";
				stsMsg="The assignment has not been assigned to this student.";
				rs1=st1.executeQuery("select status from "+schoolId+"_"+classId+"_"+courseId+"_dropbox where student_id='"+studentId+"' and work_id='"+workId+"'");
				
				if(rs1.next())
				{
					stsClr="#518F2C";
					stsMsg="The assignment has been assigned to this student.";
				}
%>
