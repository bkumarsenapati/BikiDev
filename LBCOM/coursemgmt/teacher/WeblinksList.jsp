...
try
{
	...
	rs = st.executeQuery("select * from courseweblinks where course_id= '"+courseId+"' and school_id='"+schoolId+"' and docs_id='"+materialId+"'");//modified on 2004-8-17
 %>
    ...
    &nbsp;
	<table align=center border=0 10pt; font-weight: bold\ width="600" height="34" >
  <tr><td> <a href="/LBCOM/coursemgmt/teacher/CourseFileManager.jsp?foldername=<%=materialId%>&docname=<%=docName%>&cat=<%=catId%>&workid=<%=materialId%>&tag=&status="> << Bacck to Course Material</a>
  </td></tr>
  </table>
  <table align=center border=0 10pt; font-weight: bold\ width="600" height="134" >
  
    <tr>
      <td height="30" colspan="4" width="594"><font face="Arial" size="3"><img border="0" src="../images/weblinks.gif"></font></td>
    </tr>
    <tr>
      <td height="1" colspan="4" width="594" bgcolor="#42A2E7"> 
        <p align="center">&nbsp;</td>
    </tr>
	<tr>
 ...
 {   	
   ...
   if(!turl.startsWith("http://"))
   	...
   	out.println("<tr><td height='25' width='30' align=\"center\"><font face='Arial' size='3'><input type=radio name=click value='"+t+"'></font></td>");
	//
	if(status==0)
		{
%>
		<td align="left" width="10" height="20"><a href="#" onclick="weblinkStatus('<%=t%>','no')"><font color="green"><img src="images/eye_close.png" width="20" height="20" border="0"></a></font></td>
<%
		}
		else
		{
			%>
		<td align="left" width="10" height="20"><a href="#" onclick="weblinkStatus('<%=t%>','yes')"><font color="red"><img src="images/eye_open.png" width="20" height="20" border="0"></a></font></td>
<%
		}

	//
	out.println("<td align=\"left\"  height='25' width='100'> <font face='Arial' size='2'><a href="+adturl+" target='_Blank'>"+t+"</a></font> </td>  <td align=\"left\"  height='25' width='300'> <font face='Arial' size='2'>"+adturl+"</font> </td></tr>");
	flag=true;
 }

 if(!flag)
 	out.println("<tr><td height='25' colspan='4' valign='center' ><font face='Arial' size='2'> Web Links are yet to be provided</font> </td></tr>");
%>
  
  <tr>
      <td align=center height="27" colspan="4" width="575">
...
<img border="0" src="../images/studentslistfooter.gif">
  ...
  <tr>
      <td align=center height="37" colspan="4" width="575">
<p align="center"><font face="Arial" size="3">
...
function go(tag){
	if(tag=='add')
		window.location.href="CreateWeblink.jsp?classid="+classId+"&coursename="+courseName+"&courseid="+courseId+"&classname=<%=className%>&cat=<%=catId%>&workid=<%=materialId%>";
	else if(tag=='mod'){		
		if(gettitle()==true){							window.location.href="EditWeblink.jsp?classid="+classId+"&coursename="+courseName+"&title="+title+"&courseid="+courseId+"&classname=<%=className%>&cat=<%=catId%>&workid=<%=materialId%>";
		...
		if(gettitle()==true){
			...
			window.location.href="/LBCOM/coursemgmt.AddWebLinks?mode=del&coursename="+courseName+"&classid="+classId+"&title="+title+"&courseid="+courseId+"&classname=<%=className%>&cat=<%=catId%>&workid=<%=materialId%>";
...
function weblinkStatus(webtitle,status)
{
		//window.location.href="WebLinkStatus.jsp?mode="+status+"&title="+webtitle;
		window.location.href="WebLinkStatus.jsp?mode="+status+"&title="+webtitle+"&cat=<%=catId%>&workid=<%=materialId%>";
