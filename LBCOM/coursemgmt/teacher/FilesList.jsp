...
<%
		try
		{
		   ...
		   flag=true;
		   if(cat.equals("all")){
				rs=st.executeQuery("select   curdate(),work_id,doc_name,category_id,asgncontent,modified_date,marks_total,to_date,status,topic,subtopic from "+schoolId+"_"+classId+"_"+courseId+"_workdocs where teacher_id='"+teacherId+"' order by "+sortStr+" LIMIT "+start+","+pageSize);
		   }else{
			   rs=st.executeQuery("select   curdate(),work_id,doc_name,category_id,asgncontent,modified_date,marks_total,to_date,status,topic,subtopic from "+schoolId+"_"+classId+"_"+courseId+"_workdocs where teacher_id='"+teacherId+"' and category_id='"+cat+"' order by "+sortStr+" LIMIT "+start+","+pageSize);
		   }
		   bgColor="#DBD9D5"; 
...
<!--	<td width="14" bgcolor="<%= bgColor %>" height="18" align="center" valign="middle"><a href="teacherdropbox/submittedFilesList.jsp?totrecords=&start=0&cat=<%=cat%>" target="bottompanel"><img src="../images/iassign.gif"  border="0" TITLE="Evaluate" ></a></td> 	  -->
	...
	<td width="125" height="18" bgcolor="<%= bgColor %>"><font size="2" face="Arial" color="<%=forecolor%>"><a href="#" onclick="showFile('<%= rs.getString("asgncontent") %>','<%= rs.getString("doc_name") %>','<%=cat%>');return false;"><%= rs.getString("doc_name") %></a></font></td>
    ...
    <!--<td width="101" height="18" bgcolor="<%= bgColor %>"><font size="2" face="Arial" color="<%=forecolor%>"><%=rs.getString("asgncontent") %></font></td>-->
...
%>	
  ...
  <tr>
   ...
   <table>
  <tr>
  <td align="center"><font face=arial size="2">Action on selected assignments: </font><a href="#" onclick="assign();return false;" ><font face=arial size="2">Assign</font></a>&nbsp;/&nbsp;<a href="#" onclick="makeAvailable('AM','','');return false;" ><font face=arial size="2">Make Available</font></a>&nbsp;/&nbsp;<a href="#" onclick="makeAvailable('UA','','');return false;" ><font face=arial size="2">Make Unavailable</font></a><font face=arial size="2">&nbsp; to students</font></td>
  </tr>
  </table>
  </center> 
  ...
  <!--
	function assign(){
		if(validate("ASGN")==true){
			parent.bottompanel.location.href="/LBCOM/exam/AsmtAsgn.jsp?nrec=<%=pageSize%>&examtype=<%=cat%>&checked="+checked+"&unchecked="+unchecked;
			return false;
		}
	...
	function makeAvailable(mode,examId,examName){
		if(validate("MA")==true){			
			if(mode=="UA"){
				parent.bottompanel.location.href="/LBCOM/MakeAsmtAvailable?nrec=<%=pageSize%>&cat=<%=cat%>&mode=UA&checked="+checked+"&unchecked="+unchecked;
			}else{	
				if(mode=="SL"){
					if(confirm("Modifying control information automcatically makes this assessment available to those to whom it is assigned. \n Ready to modify?")==true){
						parent.bottompanel.location.href="/LBCOM/exam/ExamControllerForBulk.jsp?examid="+examId+"&nrec=<%=pageSize%>&cat=<%=cat%>&mode="+mode+"&checked="+checked+"&unchecked="+unchecked+"&examname="+examName;
						return false;
					}else
						return false;
				}else{
