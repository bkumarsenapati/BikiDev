...
try{
	...
	{
	        ExceptionsFile.postException("SelectDates.jsp","Operations on database","SQLException",se.getMessage());
			System.out.println("Error: SQL -" + se.getMessage());
	}
	catch(Exception e)
	{
	   ExceptionsFile.postException("SelectDates.jsp","Operations on database","Exception",e.getMessage());
...
function validate()
{
	...
	{
		alert("You have to enter the group name to save the assignments!!!");
		return false;
	}
...
<form name='duedates' action="AssignManyToMany.jsp?workids=<%=widStr%>&studentids=<%=sidStr%>" method="POST" onsubmit="return validate();" >
...
<table border="0" cellpadding="0" cellspacing="0" bordercolor="#111111" width="80%" height="40">
  ...
  <tr>
    ...
    date and proceeding will distribute all the selected assignments to all the 
