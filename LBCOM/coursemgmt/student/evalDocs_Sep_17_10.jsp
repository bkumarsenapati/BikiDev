...
<%!
    int pageSize=10;		//no.of eval documents to be displayed per page	
%>
<%
    Connection con=null;
	Statement st=null;
   ...
   try
	{   
		...
		if (request.getParameter("totrecords").equals("")) { 
			...
			{
				rs=st.executeQuery("select count(distinct student_id) from "+schoolId+"_"+classId+"_"+courseId+"_workdocs w inner join "+schoolId+"_"+classId+"_"+courseId+"_dropbox d on w.work_id=d.work_id where d.student_id='"+studentId+"' and d.status >= 4");
			}
			else
			{
				rs=st.executeQuery("select count(distinct student_id) from "+schoolId+"_"+classId+"_"+courseId+"_workdocs w inner join "+schoolId+"_"+classId+"_"+courseId+"_dropbox d on    w.work_id=d.work_id where d.student_id='"+studentId+"' and d.status >= 4 and w.category_id='"+categoryId+"'");
			}
        ...
        int i=st.executeUpdate("update "+schoolId+"_"+classId+"_"+courseId+"_dropbox set status = 5 where status=4 and student_id='"+studentId+"'");
        
		/* select the required fields for displaying them to the student*/
			
		sCount="";
		if(categoryId.equals("all")){
			rs=st.executeQuery("select w.doc_name,w.category_id,w.max_attempts,w.asgncontent,w.teacher_id,w.category_id, d.answerscript submitted_file,d.submitted_date,d.eval_date,d.marks_secured, d.stuattachments from "+schoolId+"_"+classId+"_"+courseId+"_workdocs w inner join "+schoolId+"_"+classId+"_"+courseId+"_dropbox d on w.work_id=d.work_id where d.status >= 4 and d.student_id='"+studentId+"' order by w.doc_name,d.submit_count desc LIMIT "+start+","+pageSize);
		}else{
			rs=st.executeQuery("select w.doc_name,w.category_id,w.max_attempts,w.asgncontent,w.teacher_id,w.category_id, d.answerscript submitted_file,d.submitted_date,d.eval_date,d.marks_secured, d.stuattachments from "+schoolId+"_"+classId+"_"+courseId+"_workdocs w inner join "+schoolId+"_"+classId+"_"+courseId+"_dropbox d on w.work_id=d.work_id where d.status >= 4 and d.student_id='"+studentId+"' and w.category_id='"+categoryId+"' order by w.doc_name,d.submit_count desc LIMIT "+start+","+pageSize);
		}
	}
...
<tr>
  ...
  <!--  <td width="19" height="18" bgcolor="#E7E7E7" align="center" valign="middle"><font size="2" face="verdana"><input type="checkbox" name="selids" ></font></td>
	...
	<td width="20%" height="18" bgcolor="#E7E7E7">
		...
		<a href="#" onclick="go('<%=categoryId%>','<%=rs.getFloat("marks_secured")%>')">Work File</a></font>
	</td>
	<td width="20%" height="18" bgcolor="#E7E7E7">
		<font size="2" face="verdana"><a href="#" onclick="go1('<%=rs.getString("stuattachments")%>','<%=categoryId%>')">Answer File</a></font>
	</td> 
	...
	<td width="15%" height="18" bgcolor="#E7E7E7" align="center">
 	...
 	<td width="10%" height="18" bgcolor="#E7E7E7" align="center">
		<font size="2" face="verdana"><%=rs.getFloat("marks_secured")%></font>
	</td> 	 
<%     			
		}
	}catch(Exception e){
