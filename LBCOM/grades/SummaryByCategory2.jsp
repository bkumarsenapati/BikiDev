...
<%
	rs1=st1.executeQuery("select grade, username, fname, lname from studentprofile where schoolid='"+schoolId+"'  and crossregister_flag in(0,1,2) and username= any(select distinct(student_id) from coursewareinfo_det where school_id='"+schoolId+"' and course_id='"+courseId+"')");
...
<%
	if(flag==false)
	{
		out.println("</td></tr></table><font face='Arial' size='2'><center>Students are not available yet..</center></font><hr color='#F16C0A' width='90%' size='1'>");
		return;
	}
%>
</td>
...
<%
	...
	{
		ExceptionsFile.postException("SummaryByCategory2.jsp","operations on database","Exception",e.getMessage());
    }
	finally
	{
		try
		{
			...
			ExceptionsFile.postException("SummaryByCategory2.jsp","closing statement and connection  objects","SQLException",se.getMessage());
...
function goResults(obj)
{
		...
		document.location.href="SummaryByCategory3.jsp?userid=<%=teacherId%>&courseid="+cid+"&sid="+studentid;
...
function change(grade1)
{
	...
	else
	{
		alert("Select Grade")
		grades='no';
		location.href="SummaryByCategory2.jsp?userid=<%=teacherId%>&courseid="+grade1;						
	}
}
function clear1() {
		var i;
