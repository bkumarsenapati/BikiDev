...
try{
	...
	month = ""+tm;	          	      
	      ...
	      String copyBodyUrl = attachPath + "/" + schoolId + "/" + studentId + "/email";
	      ...
	      if(len>0)
	      {	  
		  String copyUrl = attachPath + "/" + schoolId + "/" + studentId + "/attachments/" + emailId;
		  ...
		  for(int i=0;i<len;i++)  
	        ...
	        rs = st.executeQuery("select count(*) allmail from mail_box where user_id='"+studentId+"'");
	    	if(rs.next())
	       		allMail = rs.getInt("allmail"); 
	    	size = 250*allMail;    
	    	size += recursiveSizecount(attachPath + "/" + schoolId + "/" + studentId + "/attachments");
	    	size += recursiveSizecount(attachPath + "/" + schoolId + "/" + studentId + "/email");
	    	size /=1048576;
...
<form name="composeform" method="post">
 <table  width="770" height="185">
       ...
       <font face="Arial" size="2" color="#E25F38">Your mail has been sent successfully!</font></b></td>
