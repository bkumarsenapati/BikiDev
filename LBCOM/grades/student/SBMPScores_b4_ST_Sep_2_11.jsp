...
<%
	...
	try
	{
		...
		rs=st.executeQuery("select * from coursewareinfo where school_id='"+schoolId+"' and course_id='"+courseId+"' and status>0 order by course_id");	
  ...
  <tr>
    ...
    <b><font face="verdana" size="2">Grade&nbsp;: </font>&nbsp;<font face="verdana"size="2" color="#990000"><b><%=grade%></font></b></td>
			...
			<%
				...
				rs2=st2.executeQuery("select sum(ntable.sum1) as sum1,sum(ntable.sum2) as sum2,ntable.category_id from ( select        `"+schoolId+"_cescores`.`marks_secured` as sum1,`"+schoolId+"_cescores`.`total_marks` as sum2,`"+schoolId+"_cescores`.category_id from `"+schoolId+"_cescores`,`mp_cescores`  where    `"+schoolId+"_cescores`.`user_id`='"+studentId+"' and `"+schoolId+"_cescores`.`course_id`='"+courseId+"'  and `"+schoolId+"_cescores`.`report_status`=1 and `"+schoolId+"_cescores`.`course_id`=`mp_cescores`.`course_id` and `"+schoolId+"_cescores`.`work_id`=`mp_cescores`.`work_id`  and `"+schoolId+"_cescores`.school_id=`mp_cescores`.school_id and `mp_cescores`.`m_id`='"+marking_id+"')  as ntable group by ntable.category_id");
...
%>			
	...
	<tr>
		<td width="25%" height="1"><font face="Arial" size="2">&nbsp;<a href="SBCScores.jsp?courseid=<%=courseId%>&m_id=<%=marking_id%>&studentid=<%=studentId%>&classid=C000&catType=<%=catId%>&catName=<%=catType%>&pcg=<%=percentage%>&sosp=<%=sumofSecuredPoints%>&sotp=<%=sumofTotalPoints%>"><%=catType%></a></font></td>
					<td width="6%" height="1" align="center">
<%
					if(grSys.equals("2"))
					{
%>
						<font face="Arial" size="2">&nbsp;<font color="red"><%=sumofTotalPoints%></a></font>
...
<%
		...
		{
			System.out.println("Error: SQL SBMPScores -" + se.getMessage());
		}
		catch(Exception e)
		{
			System.out.println("Error: SBMPScores -" + e.getMessage());
		}
%>
</table>
</center>
