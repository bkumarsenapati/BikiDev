...
try{
	...
	if(crFlag==true)
	{
		if(courseId.equals("all"))
			...
			rs=st.executeQuery("select count(distinct(use_date)) from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and class_id='"+classId+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date ");
		if(rs.next())
		...
		if(courseId.equals("all"))
	        ...
	        rs=st.executeQuery("select count(distinct(use_date)) from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date ");
		if(rs.next())
	              totRecords = rs.getInt(1);
	}	
...
<script language="javascript">
	...
	{
	        window.location.href="ByDay.jsp?studentid="+studentId+"&classid="+classId+"&start="+start+"&courseid="+courseId;
	        return false;
	}
...
<form name="dayreport">
 ...
 %>
	    ...
	    <% if((start==0)&(totRecords==0)){    %>  
	        <td width="290">  </td>
	    <%  }else{  %>			 
		<td width="290"><font color="#000080" face="arial" size="2"><%= (start+1) %> - <%= end %> of <%= totRecords %>&nbsp;Days&nbsp;&nbsp;</font>
                	   </td>
	    <% } %>  
...
<%
       ...
       if(crFlag==true)
	{
		if(courseId.equals("all"))
			...
			rs=st.executeQuery("select distinct(use_date) from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date  LIMIT "+start+","+pageSize);		   
	}else{
		if(courseId.equals("all"))
	        ...
	        rs=st.executeQuery("select distinct(use_date) from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date  LIMIT "+start+","+pageSize);		
	}     
 ...
 <% 
     ...
     {     
		if(courseId.equals("all"))
			...
			rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time) from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and use_date='"+dayId+"' and class_id='"+classId+"' and course_id='"+courseId+"' group by session_id order by use_date, use_time");
     }else{
		 if(courseId.equals("all"))
			 ...
			 rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time) from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and use_date='"+dayId+"' and class_id='"+classId+"' and course_id='"+courseId+"' group by session_id order by use_date, use_time");
     }
    while(rs2.next()){
                        sessionNo++;
