...
try{
	...
	if(crFlag==true)
	{
		if(courseId.equals("all"))
			rs=st.executeQuery("select distinct year(use_date), month(use_date), day(use_date), use_date from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and class_id='"+classId+"' and course_id like '"+schoolId+"%' order by use_date");
		else
	        rs=st.executeQuery("select distinct year(use_date), month(use_date), day(use_date), use_date from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date");
	}else{
		if(courseId.equals("all"))
			rs=st.executeQuery("select distinct year(use_date), month(use_date), day(use_date), use_date from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and class_id='"+classId+"' order by use_date");
		else
			rs=st.executeQuery("select distinct year(use_date), month(use_date), day(use_date), use_date from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date");
	}
	while(rs.next())
	{
	    newCal.clear(); 
...
<script language="javascript">
	...
	{
	       alert(start); window.location.href="ByWeek.jsp?studentid="+studentId+"&classid="+classId+"&start="+start+"&courseid="+courseId;
	        return false;
	}
	function gotopage(classId, studentId, courseId)
	{
		...
		if(page==0){
				...
				window.location.href="ByWeek.jsp?studentid="+studentId+"&classid="+classId+"&start="+start+"&courseid="+courseId;	
				return false;
			}
	}	
...
<form name="weekreport">
 ...
 %>
	    ...
	    <% if((start==0)&(totRecords==0)){    %>  
	        <td width="290">  </td>
	    <%  }else{  %>			 
		<td width="290"><font color="#000080" face="arial" size="2"><%= (start+1) %> - <%= end %> of <%= totRecords %>&nbsp;Weeks&nbsp;&nbsp;</font>
                	   </td>
	    <% } %>  
...
<table border="1" cellspacing="0" width="90%" id="AutoNumber1"  bordercolor="#E3E4FA" cellpadding="0" bordercolorlight="#808000" style="border-collapse: collapse" align="center">
 ...
 <% 
     ...
     while(stk2.hasMoreTokens()){
			...
			{
				if(courseId.equals("all"))
					rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time) from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and use_date='"+tempDate+"' and class_id='"+classId+"' and course_id like '"+schoolId+"%' order by use_date");
				else
                    rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time) from usage_detail where school_id='"+oSchoolId+"' and student_id='"+oStudentId+"' and use_date='"+tempDate+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date");
            }else{
				if(courseId.equals("all"))
					rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time) from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and use_date='"+tempDate+"' and class_id='"+classId+"' order by use_date");
				else
			        rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time) from usage_detail where school_id='"+schoolId+"' and student_id='"+studentId+"' and use_date='"+tempDate+"' and class_id='"+classId+"' and course_id='"+courseId+"' order by use_date");
			}
