...
<tr>
  ...
  <tr>
    ...
    <b><font face="verdana" size="2">Grade&nbsp;: </font>&nbsp;<font face="verdana"size="2" color="#990000"><b><%=grade%></font></b></td>
	 ...
	 rs2=st2.executeQuery("select sum(ntable.sum1) as sum1,sum(ntable.sum2) as sum2,ntable.category_id from ( select        `"+schoolId+"_cescores`.`marks_secured` as sum1,`"+schoolId+"_cescores`.`total_marks` as sum2,`"+schoolId+"_cescores`.category_id from `"+schoolId+"_cescores`,`mp_cescores`  where    `"+schoolId+"_cescores`.`user_id`='"+studentId+"' and `"+schoolId+"_cescores`.`course_id`='"+courseId+"'  and `"+schoolId+"_cescores`.`report_status`=1 and `"+schoolId+"_cescores`.`status`<3 and `"+schoolId+"_cescores`.`category_id`!='ST' and `"+schoolId+"_cescores`.`course_id`=`mp_cescores`.`course_id` and `"+schoolId+"_cescores`.`work_id`=`mp_cescores`.`work_id`  and `"+schoolId+"_cescores`.school_id=`mp_cescores`.school_id and `mp_cescores`.`m_id`='"+marking_id+"')  as ntable group by ntable.category_id");
