...
try{
	   ...
	   if(crFlag==true)
	{
		...
		System.out.println("select count(distinct(use_date)) from usage_teach_detail where school_id='"+schoolId+"' and teacher_id='"+teacherId+"' and class_id='"+classId+"' order by use_date, use_time");
	...
	totalTime = studyTotal = asmtTotal = asgnTotal = eclassTotal = chatTotal= otherTotal = sessionNo = 0;
     ...
     {      
	   //rs2=st2.executeQuery("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time),session_id from usage_teach_detail where school_id='"+oSchoolId+"' and teacher_id='"+oStudentId+"' and use_date='"+dayId+"' and class_id='"+classId+"' and course_id='"+courseId+"' group by session_id order by use_date, use_time");
		 ...
		 System.out.println("...............................");
		 System.out.println("select sum(study_time), sum(asmt_time), sum(asgn_time), sum(eclass_time), sum(chat_time), sum(other_time),session_id,course_id from usage_teach_detail where school_id='"+schoolId+"' and teacher_id='"+teacherId+"' and use_date='"+dayId+"' and class_id='"+classId+"'  group by session_id order by use_date, use_time");
    ...
    while(rs2.next())
	{
			...
			if(spacePos1>5)
			{
				...
				System.out.println("spacePos1.."+spacePos1+"spacePos2..."+spacePos2);
					...
					System.out.println("select other_time from usage_teach_detail where school_id='"+schoolId+"' and teacher_id='"+teacherId+"' and class_id='"+classId+"' and course_id='"+courseId+"' and use_date='"+dayId+"' and session_id='"+sessionId+"'");
					}
