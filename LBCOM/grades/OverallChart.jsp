...
<title>Overall Summary Chart</title>
...
<%  
			...
			{
%>
				<a href="OverallChart.jsp?sortby=slno&sorttype=A&courseid=<%=courseId%>&userid=<%=teacherId%>&sid=<%=studentId%>&status=" target="_self">
...
<%   
			...
			{
%>
				<a href="OverallChart.jsp?sortby=slno&sorttype=D&courseid=<%=courseId%>&userid=<%=teacherId%>&sid=<%=studentId%>&status=" target="_self">
...
%>
			...
			{
%>  
				<a href="OverallChart.jsp?&sortby=md&sorttype=A&courseid=<%=courseId%>&userid=<%=teacherId%>&sid=<%=studentId%>&status=" target="_self">
<img border="0" src="images/sort_dn_1.gif" width="12" height="11"></a>
<%   
			}
			else
			{
%>     
				<a href="OverallChart.jsp?sortby=md&sorttype=D&courseid=<%=courseId%>&userid=<%=teacherId%>&sid=<%=studentId%>&status=" target="_self">
					<img border="0" src="images/sort_up_1.gif" width="12" height="11"></a>
<%   
			}
%>
...
<table border="0" bordercolor="green"><tr><td>&nbsp;</td></tr></table>
 ...
 <div id="overallchartdiv1">
   ...
   <script language="JavaScript"> 
	...
	var chartbyatt2 = new FusionCharts("../FusionCharts/Pie3D.swf", "chart1Id", "500", "200", "0", "1"); 
...
<%
	...
	{
		ExceptionsFile.postException("OverallChart.jsp","operations on database","Exception",e.getMessage());
    }
	finally
	{
		try
		{
			...
			ExceptionsFile.postException("OverallChart.jsp","closing statement and connection  objects","SQLException",se.getMessage());
...
function goResults(obj)
{
		...
		document.location.href="OverallChart.jsp?userid=<%=teacherId%>&courseid="+cid+"&sid="+studentid;
