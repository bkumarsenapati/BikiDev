...
<%
	...
	String developerId="",courseId="",courseName="",courseDevPath="",aCourseId="",aTeacherId="",aSchoolId="";
	String aMaterialId="",aCatId="",folderName="",docName="";
...
try{
	...
	aCatId=request.getParameter("catid");
	aMaterialId=request.getParameter("materialid");
...
function validate()
{
	...
	if(size==0)
	{
		alert("There are no courses available!");
//		return;
	}
...
for(var i=0;i<size;i++)
	{
		...
		{   
			cname=window.document.dir.elements[i].value;

			if(confirm("Are you sure that you want to import \""+cname+"\" course?")==true)
			{
				bool==1;
				window.location.href="CourseCopy.jsp?foldername=<%=folderName%>&docname=<%=docName%>&coursename="+cname+"&catid=<%=aCatId%>&materialid=<%=aMaterialId%>&courseid=<%=aCourseId%>&teacherid=<%=aTeacherId%>&schoolid=<%=aSchoolId%>";
	...
	if(bool==0)
	{
		alert("Please select any course to import!");
		return;
	}
...
<tr>
	<td width="100%" height="100%" colspan="3" background="images/bg2.gif" align="center" valign="top">
		...
		<th height="86" align="center" valign="top" scope="col">
	        ...
	        <tbody>
		    <tr>
				<td width="390" align="middle" class="Grid_tHeader" height="20" valign="left" colspan="2"><b>My Courses</b></td>
...
%>
		<tr>
		  ...
		  <td width="21"  bgcolor="#FFFFFF"><input type="radio" name="dir" value="<%=courseName%>" TITLE="<%=courseName%>"></td>
		  <td align="left" class="tr-subrow" width="362" height="20"><font color="#34526c"><%=courseName%></font></a>
		</td>
...
%>
	...
	<tr>
		<td align="center"><input type="button" value="Submit" onclick="validate();" name="sb"></td>
	</tr>
    </table>
	<br /></div>
	</td>
