...
<%
	...
	String examName="",queryString="",developerId="";
%>
<%
	...
	try
	{
		...
		rs1=st1.executeQuery("select count(*) from lbcms_dev_assessment_master where course_id='"+courseId+"'");
		if(rs1.next())
		{
		...
		rs=st.executeQuery("select slno,assmt_id,assmt_name  from lbcms_dev_assessment_master  where  course_id='"+courseId+"' order by slno  LIMIT "+start+","+pageSize);
...
<tr>
	...
	<td width="368" height="70">
		<img src="images/logo.png" width="368" height="101" >
	</td>
...
<td width="40%" align="right" height="20" class="gridhdrNew"><a href="ViewAssessInfo.jsp?mode=none&courseid=<%=courseId%>&coursename=<%=courseName%>&userid=<%=developerId%>"><font color="blue">&lt;&lt; Back To Manage Assessments&nbsp;</font></a></td>
</tr>
</table>
<table width="90%"  cellspacing="0" cellpadding="0" class="gridhdrNew1">
<hr>
<tr class="gridhdrNew1">
...
<table  cellpadding="1" cellspacing="0" width="90%" class="gridhdrNew1">
  <tr class="gridhdrNew1">
  	<td width="1%" align="center" class="gridhdrNew1">&nbsp;</td> 
    <td width="94%" class="gridhdrNew1"><b>
    <font face="Verdana" size="2" >&nbsp;&nbsp;Assessment Name</font></b></td>
...
%>  
  <form name="ordering<%=i%>" method="POST" action="OrderAssessments2.jsp?courseid=<%=courseId%>&cname=<%=courseName%>&uid=<%=developerId%>">
  <tr class="gridhdrNew1" onMouseover="bgColor='#cccccc'" onMouseOut="bgColor='#EEEEEE'">
	 ...
	 <td width="91%"><font face="Verdana" size="2">&nbsp;<%=examName%></font></td>
	<td width="94%"><select name="cv"><option value="">Select</option>
	<%
	  int x=0;

	  rs2=st2.executeQuery("select slno from lbcms_dev_assessment_master where course_id='"+courseId+"' order by slno");
		while(rs2.next())
		{
