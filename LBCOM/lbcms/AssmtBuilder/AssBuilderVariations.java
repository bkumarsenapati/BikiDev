...
public class  AssBuilderVariations extends HttpServlet
{
	public void init(ServletConfig conf) {
		...
		{
			ExceptionsFile.postException("AssBuilderVariations.java","init","Exception",e.getMessage());
		}
	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws IOException,ServletException{
		...
		Statement st=null;
		ResultSet rs=null;
		AssBuilderExamPaper examPaper=null;
        ...
        try{
			...
			System.out.println("after thee else..."+schoolPath);

			
			db=new DbBean();
			examPaper=new AssBuilderExamPaper();	
			...
			qtnTbl="dev_assmt_content_quesbody";
			...
			rs=st.executeQuery("select * from dev_assessment_master where assmt_id='"+examId+"'");
			while (rs.next()){
				//createDate=rs.getString("create_date").replace('-','_');				
				questionString=rs.getString("ques_list");	
			}
			System.out.println("after thee else...after while loop");
			...
			//st.executeBatch();	
			System.out.println("after thee else...");
			examPaper.setPaper(qList,examId,size,teacherId,qtnTbl,schoolId,courseId,courseName,schoolPath,0);
			System.out.println("after examPaper.setPaper..");
			out.println("<script>");
			out.println("window.location.href=\"/LBRT/coursemgmt/teacher/AsgnFrames.jsp?totrecords=&start=0&checked=&unchecked&cat=edit&workid="+examId+"&docname="+examName+"\";");
		 
			//out.println("parent.parent.top_fr.location.href=\"/LBRT/exam/CETopPanel.jsp?status=4&editMode=edit&examId="+examId+"&examName="+examName+"&examType="+examType+"&noOfGrps="+noOfGrps+"\";");

			//path=schoolPath+"/"+courseId+"/"+examId+"/";
			path="/LBRT/coursedeveloper/AssmtBuilder/"+courseId+"/"+examId+"/";
			System.out.println("path.."+path);
			out.println("window.location.href=\"/LBRT/coursedeveloper/AssmtBuilder/QuestionsList.jsp?view=yes&courseid="+courseId+"&exId="+examId+"\";");
			out.println("</script>");	

		}catch(Exception e){
			ExceptionsFile.postException("AssBuilderVariations.java","doPost","Exception",e.getMessage());
		}finally{
			try{
				 if(st!=null)
					 st.close();
				 ...
				 sortedList=null;
				        ...
				        ExceptionsFile.postException("AssBuilderVariations.java","closing connections","SQLException",se.getMessage());
               }
		}
	}
	...
	private String sortGroupWise(String quesString,int size){
		try	{
			...
			ExceptionsFile.postException("AssBuilderVariations.java","sortGroupWise","Exception",e.getMessage());
		}
		return quesString;
	}
	private String randomize(Hashtable quesList){
		...
		try{
			...
			ExceptionsFile.postException("AssBuilderVariations.java","randomize","Exception",e.getMessage());
		}
		return qString;
	}
	private int getSize(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("AssBuilderVariations.java","getSize","Exception",e.getMessage());
		}finally{
			stkQues=null;
		}
		return size;
	...
	private Hashtable getQuestionsList(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("AssBuilderVariations.java","getQuestionsList()","Exception",e.getMessage());
		}finally{
			stkQues=null;
		}
		return quesList;
	...
	private Hashtable formQuestion(String quesString){
		...
		try{
			...
			ExceptionsFile.postException("AssBuilderVariations.java","formQuestion","Exception",e.getMessage());
		}
		return grpsList;
	}
	...
	private Hashtable sortQuesWise(Statement st,ResultSet rs,Hashtable questionsList,String schoolId,String classId,String courseId){
		...
		try{
			...
			if(queryString.length()>0){
				...
				rs=st.executeQuery("select q_id,q_type from dev_assmt_content_quesbody where ("+queryString+")");
				while(rs.next()){
					qid=rs.getString("q_id");
					qtype=rs.getString("q_type");
					System.out.println("qtype....Variations..."+qtype);
			...
			ExceptionsFile.postException("AssBuilderVariations.java","sortQuesWise","Exception",e.getMessage());
		}
		return sortedList;
		//return ques;
	  }
	  ...
	  private Vector getSortedList(Hashtable qList){
		  ...
		  try{
			...
			Collections.sort(list);
				...
				ExceptionsFile.postException("AssBuilderVariations.java","getSortedList","Exception",e.getMessage());
		 }
	  ...
	  private String getSortedQuesString(Hashtable qList,Vector list){
		  ...
		  try{
			...
			//	sortedList.put(i,qList.get(i));
				...
				ExceptionsFile.postException("AssBuilderVariations.java","getSortedQuesString","Exception",e.getMessage());
		  }
		return q;
	  }
		  ...
		  try{
			 ...
			 // Hashtable qList=new Hashtable();
			  ...
			  while(ele.hasMoreElements()){
				...
				ExceptionsFile.postException("AssBuilderVariations.java","getSortedHashtable","Exception",e.getMessage());
		  }
		  return sortedList;
	  }
