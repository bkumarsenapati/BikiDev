...
%>
	<form method="post"  name="seciconsall" action="UpdateSecIcons.jsp?mode=add&userid=<%=developerId%>&courseid=<%=courseId%>&coursename=<%=courseName%>&unitid=<%=unitId%>&unitname=<%=unitName%>&lessonid=<%=lessonId%>&lessonname=<%=lessonName%>&sectitle=<%=secTitle%>">
...
<%
	 int i=1;
		...
		<%
			...
			<table>
			<tr>
			<td width="1%" colspan="4"><input type="checkbox" name="updatestatus" value="no" onclick="getValue(this.id)">
							</td> 
							</tr>
			<tr>
				<td>
