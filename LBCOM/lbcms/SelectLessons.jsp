...
<%
	...
	String gradeId="",uName="",uId="",statusMsg="";
	boolean sflag=false;
	...
	try
	{
		...
		unitId=(String)session.getAttribute("cb_unitid");
		unitName=(String)session.getAttribute("cb_unitname");
		
		lessonId=request.getParameter("lessonid");
		lessonName=request.getParameter("lessonname");
		gradeId=request.getParameter("gradeid");
		
		statusMsg=request.getParameter("msg");
		if(statusMsg==null)
		{
			statusMsg="";
		}
		...
		{
			statusMsg="Lesson(s) added successfully";
		}
...
{	
	...
	window.location="SelectLessons.jsp?userid=<%=developerId%>&courseid=<%=courseId%>&coursename=<%=courseName%>&unitid=<%=unitId%>&unitname=<%=unitName%>&lessonid=<%=lessonId%>&lessonname=<%=lessonName%>&gradeid="+unitId+"&standardid="+uId;		
...
function validate()
{
	...
	for(i=0;i<obj.elements.length;i++)
	{
		...
		{
			lessonids[i]=obj.elements[i].value;
			stdCount=stdCount+1;
		}
	}
	if(stdCount < 1)
	{
		alert("You have to select atleast one lesson!!!");
		return false;
	}
	else
	{
		...
		window.location.href="CrtLessonArchives.jsp?userid=<%=developerId%>&courseid=<%=courseId%>&unitid=<%=unitId%>&lessonid=<%=lessonId%>&lessonids="+lessonids;
		return false;
	}
...
function saveLessons()
{
	...
	else
	{
		...
		window.location.href="LessonArchivesSave.jsp?mode=save&userid=<%=developerId%>&ocourseid="+unitId+"&unitid=<%=unitId%>&lessonid=<%=lessonId%>&lessonids="+lessonids;
		return false;
	}
        ...
        <tr  class="gridhdrNew">
               <td align="left" width="419" height="30" valign="middle">
					...
					&gt;&gt; <a href="CourseUnits.jsp?userid=<%=developerId%>&courseid=<%=courseId%>&coursename=<%=courseName%>">
						<%=courseName%></a>
					&gt;&gt; <a href="CourseUnitLessons.jsp?userid=<%=developerId%>&courseid=<%=courseId%>&coursename=<%=courseName%>&unitid=<%=unitId%>&unitname=<%=unitName%>"><%=unitName%>&nbsp;&nbsp;>>Back to Lessons</b></a></td>
...
<table>
<tr>
	<td width="1%" align="center" >&nbsp;<input class="button" type="button" value="Submit" align="absmiddle" onClick="return validate('<%=courseId%>');"></td>
</tr>
...
<table width="70%">
<tr>
<td width="60%" align="center">&nbsp;<font color="green"><%=statusMsg%></font></td>
</tr>
</table>
		 </form></body></html>