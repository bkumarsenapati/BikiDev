...
//	System.out.println("at 1....import.aSchoolId...."+aSchoolId);
...
//		if(window.document.dir.elements[i].checked)
		...
		{   
			if(confirm("Are you ready to import the assignments?")==true){
				...
				//alert(cid);
				bool==1;
				return window.location.href="/LBCOM/lbcms/ImpCreateAssessments.jsp?courseid="+cid+"&acourseid=<%=aCourseId%>&ateacherid=<%=aTeacherId%>&aschoolid=<%=aSchoolId%>";
				}
			else
				return false;
		}
	...
	if(bool==0)
	{
		alert("Select an item to import.");
		return;
	}
...
%>
		<tr>
		  ...
		  <td align="left" class="tr-subrow" width="362" height="20"><font color="#34526c"><a href="SelectAssessments.jsp?courseid=<%=courseId%>&coursename=<%=courseName%>&acourseid=<%=aCourseId%>&ateacherid=<%=aTeacherId%>&aschoolid=<%=aSchoolId%>"><%=courseName%></font></a>
		</td>
