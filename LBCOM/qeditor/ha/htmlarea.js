...
HTMLArea.Config = function () {
	...
	this.btnList = {
		...
		insertimage: [ "Insert Image/MM File", "ed_image.gif", false, function(e) {e.execCommand("insertimage");} ],
		inserttable: [ "Insert Table", "insert_table.gif", false, function(e) {e.execCommand("inserttable");} ],
		htmlmode: [ "Exit HTML Editor", "ed_html.gif", true, function(e) {e.execCommand("htmlmode");} ],
	...
	for (var i in this.config.toolbar) {
		...
		for (var j in group) {
			...
			if (/^([IT])\[(.*?)\]/.test(code)) {
				...
				var l7ed = RegExp.$1 == "I"; // locali~hsn?
				var label = RegExp.$2;
				if (l7ed) {
...
HTMLArea.prototype.generate = function () {
	editor = this;	// we'll need "this" in some nested functions
	// get the textarea
	...
	function initIframe() {
		...
		if (!doc) {
			...
			if (HTMLArea.is_gecko) {
				...
				alert("ERROR: IFRAME can't be initiali~hsn.");
			}
		}
		if (HTMLArea.is_gecko) {
		...
		if (!editor.config.fullPage) {
			...
			if(document.all)
				html += "<style type=\"text/css\">*{font-family: Lucida Sans Unicode, Math1, Math2, Math3, Math4, Math5, MT Extra, CMSY10, Symbol;}</style>"
			html += "</head>\n";
			html += "<body>";
////			html += "<body>\n";
...
//////alert(document.getElementById(IF_ID).id);
};
...
// not removing it might result in memory leakage
////    e = null;
    if (editor_ta_id != '') editor_ta_id = '';
...
//alert("plugin_file: "+plugin_file);
//alert("plugin_lang: "+plugin_lang);
};
...
////~hsn							value = editor.stripBaseURL(value);
						}
					}
