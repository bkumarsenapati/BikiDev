...
Htmldata="<br><div id='div" + sid  + "' style='display:none'><table align='center' border='1' cellpadding='0' cellspacing='0' width='100%' bordercolor='#EEBA4D' bordercolordark='#EEBA4D' bordercolorlight='#EEBA4D'>";
...
String query2="select message,trans_date,user_id,sno,status,forumattachments from forum_post_topic_reply where forum_id='"+forumid+"' and topic='"+temp+"' and school_id='"+schoolid+"' and trans_type='2'  and topic_id='"+sid+"'";
		...
		while(rs3.next())
		{
			...
			Htmldata= Htmldata + "<tr>        <td align='center' valign='top' height='25' colspan='2'>            <table align='center' cellspacing='0' width='100%' bordercolordark='#E3E3E3' bordercolorlight='#E3E3E3' style='border-collapse:collapse;' cellpadding='0'>                <tr>                    <td width='100%' bgcolor='white' style='border-width:1; border-color:rgb(227,227,227); border-style:double;' height='12' align='left'>                        <table align='center' border='0' cellpadding='4' cellspacing='0' width='100%' bordercolor='#EEBA4D' bordercolordark='#EEBA4D' bordercolorlight='#EEBA4D' height='61'>                            <tr>                                <td width='184' align='left' valign='top' bgcolor='#E1E1E1'>                                    <p><font face='Arial' color='#546878'><span style='font-size:9pt;'>on " + replydate + "</span></font>&nbsp;<a href='/LBCOM/schoolAdmin/Forums/PostReply.jsp?sno=" + sno+ "&topicid=" +  topicid+ "&fid=" +  forumid + "&fname=" + dir+ "&topic=" +  topic+ "&user=" + userid+ "&postdate=" + posteddate+ "&auser=" + posteduser+ "&puser=" + posteduser+ "&crtby=" + crtby+ "&utype=" + utype+ "&desc=" + desc+ "&topiccount=" + topiccount+ "&replycount=" + replycount+ "&ruser=" + ruser+ "&replydate=" + replydate+ "'><img src='images/icon_threaded_reply.gif' width='18' height='9' border='0'></a></p>                                </td>                                <td width='361' align='left' valign='top' bgcolor='#EEEEEE'>                                    <p><font face='Arial'><span style='font-size:10pt;'><b>Re: " +  topic1 + "</b></span></font></p>                                </td>                                <td width='361' align='left' valign='top' bgcolor='#EEEEEE'>";
			...
			Htmldata= Htmldata + "&nbsp;<a href='/LBCOM/forums.DeleteForum?mode=delete&sno="+ sno +"&topicid=" + topicid + "&sid="+ schoolid +"&fid="+ forumid +"&topic="+ topic1 +"&user="+ ruser +"&postdate="+  posteddate  +"&dir="+  dir  +"&auser="+ posteduser +"&puser=" + posteduser+ "' onclick=\"return confirm('Are you sure that you want to delete this Reply?')\"><img src='../../forums/images/button_delete_thread.gif' TITLE='Delete this Thread ' border=0></a>";
			}
...
String topic="",topic1="",message1="",reply1="",topicid="",sno1="";
String posteddate=" ";
String posteduser="";
String userid="",temp="";
...
<table align="center" border="0" cellpadding="0" cellspacing="5" width="100%" bordercolor="black" bordercolordark="black" bordercolorlight="black">
    <tr>
        <td width="50%">
            <P><a href="/LBCOM/schoolAdmin/Forums/PostReply.jsp?sno=<%=sno1%>&topicid=<%= topicid%>&fid=<%= forumid %>&fname=<%=dir%>&topic=<%= temp1%>&user=<%=userid%>&postdate=<%=posteddate%>&auser=<%=posteduser%>&puser=<%=posteduser%>&crtby=<%=crtby%>&utype=<%=utype%>&desc=<%=desc%>&topiccount=<%=topiccount%>&replycount=<%=replycount%>&ruser=<%=ruser%>&replydate=<%=replydate%>"><img src="images/button_topic_reply.gif" width="96" height="25" border="0"></a></P>
        </td>
        <td width="50%">&nbsp;</td>
    </tr>
</table>
...
try
{
	String query1="select message,trans_date,user_id,sno,status,forumattachments from forum_post_topic_reply where forum_id='"+forumid+"' and topic='"+temp+"' and school_id='"+schoolid+"' and trans_type='2' and topic_id='" + sno1 +"' order by sno desc";
...
<table align="center" border="8" cellpadding="0" cellspacing="0" width="100%" bordercolor="#EEBA4D" bordercolordark="#EEBA4D" bordercolorlight="#EEBA4D">
                ...
                <tr>
                    <td width="100%" bgcolor="white" style="border-width:1; border-color:rgb(227,227,227); border-style:double;" height="12" align="left">
                        <table align="center" border="0" cellpadding="4" cellspacing="0" width="100%" bordercolor="#EEBA4D" bordercolordark="#EEBA4D" bordercolorlight="#EEBA4D" height="61">
                            <tr>
                                <td width="184" align="left" valign="top" bgcolor="#E1E1E1">
                                    <p><font face="Arial" color="#546878"><span style="font-size:9pt;">on <%=replydate%></span></font>&nbsp;<a href="/LBCOM/schoolAdmin/Forums/PostReply.jsp?sno=<%=sno%>&topicid=<%= topicid%>&fid=<%= forumid %>&fname=<%=dir%>&topic=<%= temp1%>&user=<%=userid%>&postdate=<%=posteddate%>&auser=<%=posteduser%>&puser=<%=posteduser%>&crtby=<%=crtby%>&utype=<%=utype%>&desc=<%=desc%>&topiccount=<%=topiccount%>&replycount=<%=replycount%>&ruser=<%=ruser%>&replydate=<%=replydate%>"><img src="images/icon_threaded_reply.gif" width="18" height="9" border="0"></a></p>
                                </td>
                                <td width="361" align="left" valign="top" bgcolor="#EEEEEE">
	...
	<%
			if(userid.equals("Admin") || userid.equals(posteduser) || userid.equals(ruser))
			{
			%>&nbsp;<a href="/LBCOM/forums.DeleteForum?mode=delete&sno=<%=sno%>&topicid=<%= topicid%>&sid=<%=schoolid%>&fid=<%=forumid%>&topic=<%=topic1%>&user=<%=ruser%>&postdate=<%= posteddate %>&dir=<%= dir %>&auser=<%=posteduser%>&puser=<%=posteduser%>" onclick="return confirm('Are you sure that you want to delete this Reply?')"><img src="../../forums/images/button_delete_thread.gif" TITLE="Delete this Topic " border=0></a>
			<%}
			if(attachFile.equals("") || attachFile==null || attachFile.equals("null"))
			{
...
%>
				  &nbsp;Santhosh<a href="javascript://" onclick="return showFile('<%=attachFile%>','<%=ruser%>');"><img src="../../forums/images/button_attach.gif" TITLE="Attachments" border=0></a></font>
