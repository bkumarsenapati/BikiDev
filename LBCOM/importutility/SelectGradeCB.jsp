...
<%
	...
	teacherId="teacher11";
	//out.println("select ci.class_id,class_des from coursewareinfo as ci inner join class_master as cm on ci.class_id=cm.class_id and ci.school_id=cm.school_id where ci.school_id='"+schoolId+"' group by ci.class_id");
...
<% 	
	rs=st.executeQuery("select * from lbcms_dev_course_master where status=1 order by course_name");
	int i=0,j=1;
	while (rs.next()) {
%>
		courses["<%=i%>"]=new Array('C000','teacher11','<%=rs.getString("course_id")%>','<%=rs.getString("course_name")%>');
...
<%
		...
		ExceptionsFile.postException("SelectGradeCB.jsp","operations on database","Exception",e.getMessage());
    }finally{
		try{
			if(st!=null)
				st.close();
			...
			ExceptionsFile.postException("SelectGradeCB.jsp","closing statement and connection  objects","SQLException",se.getMessage());
...
//		var courseId=document.qtnselectfrm.courseid.value;
	    ...
	    parent.main.location.href = "/LBCOM/importutility/ShowAssessmentListCB.jsp?classid="+classId+"&teacherid="+teacherId+"&courseid="+courseId+"&classname="+className+"&coursename="+courseName;
