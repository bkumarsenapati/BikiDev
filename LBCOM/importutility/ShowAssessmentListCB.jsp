...
try{
	...
	rs1 = st1.executeQuery("select bundle_id,title from cb_bundles_list where school_id='"+schoolid+"'");
	...
	try{
			...
			ExceptionsFile.postException("ShowAssessmentListCB.jsp","closing statement and connection  objects","SQLException",se.getMessage());
...
<html>
  <head>
   <!-- <base href="">-->
    <title>My JSP 'ShowAssessmentListCB.jsp' starting page</title>
	<SCRIPT LANGUAGE="JavaScript" src="/LBCOM/common/Validations.js"></SCRIPT>
	<SCRIPT LANGUAGE="JavaScript">
	<!--
	function delete1(bid){
		<%
		String url="deletebundlecb.jsp?teacherid="+teacherid+"&courseid="+courseid+"&schoolid="+schoolid+"&classid="+classid+"&topicid=&subtopicid=&classname="+classname+"&coursename="+coursename+"";	
		%>
  ...
  <form action="/LBCOM/importutility/uploadzipCB.jsp?schoolid=<%=schoolid%>&classid=<%=classid%>&courseid=<%=courseid%>&topicid=<%=topicid%>&subtopicid=<%=subtopicid%>&classname=<%=classname%>&coursename=<%=coursename%>&teacherid=<%=teacherid%>" method="post">
  <table align = center >
...
<%
   try{
	    ...
	    while(rs1.next()){
			...
			flag=true;			
			rs2=st2.executeQuery("select status from cb_course_bundles where bundle_id='"+bId+"' and school_id='"+schoolid+"' and course_id='"+courseid+"'");		 
%>
	...
	<tr>
		...
		<%
			...
			if (rs2.next()){
				...
				if (status==1){ 						
				acTag="/LBCOM/exam/GenerateExamsCB.jsp?bundleid="+bId+"&teacherid="+teacherid+"&courseid="+courseid+"&schoolid="+schoolid+"&classid="+classid+"&topicid=&subtopicid=&classname="+classname+"&coursename="+coursename;
					acTitle="Questions imported  ( <a href='javascript:import1(\""+acTag+"\");'>create assessments</a> )";
				}
				else if(status==2){
				acTag="/LBCOM/exam/ExamControllerForBulkCB.jsp?teacherid="+teacherid+"&mode=IU&coursename="+coursename+"&courseid="+courseid+"&schoolid="+schoolid+"&classid="+classid+"&classname="+classname;
				...
				rs2.close();
			} else{							acTag="/LBCOM/importutility.GetAssessmentListCB2?bundleid="+bId+"&schoolid="+schoolid+"&classid="+classid+"&courseid="+courseid+"&topicid="+topicid+"&subtopicid="+subtopicid+"&classname="+classname+"&teacherid="+teacherid+"&coursename="+coursename;
