...
function check(){
 ...
 <DIV id=loading  style='WIDTH:100%;height:100%; POSITION: absolute; TEXT-ALIGN: center;border: 3px solid;z-index:1;visibility:hidden'><IMG src="/LBCOM/common/images/loading.gif" border=0></DIV>
  <body>
	<form name="quesimport" method="post" enctype="multipart/form-data"  action="/LBCOM/importutility.uploadzipCB?schoolid=<%=schoolid%>&classid=<%=classid%>&classname=<%=classname%>&courseid=<%=courseid%>&coursename=<%=coursename%>&teacherid=<%=teacherid%>">
		<table align = center>
