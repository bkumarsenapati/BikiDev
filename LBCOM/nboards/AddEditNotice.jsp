...
function checkfld(mode)
{
	...
	*/
	...
	// Santhosh added from here
	 ...
	 var total="";
        for(var i=0; i < win.courses.length; i++)
        {
            if(win.courses[i].checked)
            {
                total +=win.courses[i].value + ",";
            }
        }
        if(total=="")
        {
...
%>
			<tr>
				...
				<td width="55%" align="left" valign="top">
					<table>
					<!-- <select id="cid" name="cid" onclick="formAction()" value="this.value();"> -->
...
<%
				...
				String chkStatus="false",cId="";
				while(rs.next())
				{
					cId=rs.getString("course_id");
					System.out.println("courseId...."+courseId+"...cId..."+cId);
...
%>
					...
					<%=alt1%> <td><input align="left" type="checkbox" name="courses" value="<%=rs.getString("course_id")%>"> <%=rs.getString("course_name")%></td><%=alt2%>
...
<%
				...
				String chkStatus="",cId="";
				while(rs.next())
				{
					cId=rs.getString("course_id");
					System.out.println("Edit......courseId...."+courseId+"...cId..."+cId);
					if(cId.equals(courseId))
					{
						chkStatus="checked";
					}
...
%>
					...
					<%=alt1%> <td><input align="left" type="checkbox" name="courses" value="<%=rs.getString("course_id")%>" <%=chkStatus%>> <%=rs.getString("course_name")%></td><%=alt2%> 				
<%				
